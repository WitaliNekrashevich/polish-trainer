...
function itemFamilySignature(item) {
  if (!item) return "";
  if (item.type === "input") {
    const question = String(item.q || "");
    const baseMatch = question.match(/\(([^()]*)\)\.?\s*$/);
    if (baseMatch) return `input-family|${norm(baseMatch[1])}`;
    return `input-family|${(item.a || []).map(norm).join("|")}`;
  }
  if (item.type === "choice") {
    return `choice-family|${norm(item.correct)}|${norm(String(item.q || "").replace(/^(что|które|wybierz|dlaczego|почему).*?:/i, ""))}`;
  }
  if (item.type === "free") return `free-family|${norm(item.q)}`;
  if (item.type === "cloze") {
    const blanks = (item.blanks || []).map((blank) => (blank.answers || []).map(norm).join("&")).join("|");
    return `cloze-family|${blanks}`;
  }
...
const makeExercise = (title, items) => {
  if (/^Разговор/i.test(String(title || ""))) return null;
  return { title, items: uniqueExerciseItems(items) };
};
...
function takeExerciseSlot(items, baseStart = 0, chunkSize = EXERCISE_MAX) {
  const unique = synthesizeExerciseItems(items);
  if (unique.length === 0) return [];
  const maxItems = Math.min(chunkSize, unique.length);
  if (unique.length <= maxItems) return shuffle(unique).slice(0, maxItems);
  const ordered = stableSortExerciseItems(unique);
  const start = (baseStart + getVariantIndex() * maxItems) % ordered.length;
  const picked = [];
  for (let i = 0; i < maxItems; i += 1) picked.push(ordered[(start + i) % ordered.length]);
  return shuffle(picked);
}
...
function buildPluralCasePool(entries, config) {
  const items = [];
  entries.forEach((entry, index) => {
    const wrong = entry.wrong || entry.base;
...
const pluralAdjectivePatterns = [
  { key: "dobry", m: "dobry", f: "dobra", n: "dobre", nomMasc: "dobrzy", nomNonMasc: "dobre", genLoc: "dobrych", dat: "dobrym", inst: "dobrymi" },
  { key: "nowy", m: "nowy", f: "nowa", n: "nowe", nomMasc: "nowi", nomNonMasc: "nowe", genLoc: "nowych", dat: "nowym", inst: "nowymi" },
  { key: "miły", m: "miły", f: "miła", n: "miłe", nomMasc: "mili", nomNonMasc: "miłe", genLoc: "miłych", dat: "miłym", inst: "miłymi" },
  { key: "ważny", m: "ważny", f: "ważna", n: "ważne", nomMasc: "ważni", nomNonMasc: "ważne", genLoc: "ważnych", dat: "ważnym", inst: "ważnymi" },
  { key: "ciekawy", m: "ciekawy", f: "ciekawa", n: "ciekawe", nomMasc: "ciekawi", nomNonMasc: "ciekawe", genLoc: "ciekawych", dat: "ciekawym", inst: "ciekawymi" },
  { key: "młody", m: "młody", f: "młoda", n: "młode", nomMasc: "młodzi", nomNonMasc: "młode", genLoc: "młodych", dat: "młodym", inst: "młodymi" }
];

const pluralMascPersonalNouns = [
  { sg: "student", nomPl: "studenci", genPl: "studentów", datPl: "studentom", instPl: "studentami", locPl: "studentach" },
  { sg: "pracownik", nomPl: "pracownicy", genPl: "pracowników", datPl: "pracownikom", instPl: "pracownikami", locPl: "pracownikach" },
  { sg: "lekarz", nomPl: "lekarze", genPl: "lekarzy", datPl: "lekarzom", instPl: "lekarzami", locPl: "lekarzach" },
  { sg: "klient", nomPl: "klienci", genPl: "klientów", datPl: "klientom", instPl: "klientami", locPl: "klientach" },
  { sg: "turysta", nomPl: "turyści", genPl: "turystów", datPl: "turystom", instPl: "turystami", locPl: "turystach" },
  { sg: "sąsiad", nomPl: "sąsiedzi", genPl: "sąsiadów", datPl: "sąsiadom", instPl: "sąsiadami", locPl: "sąsiadach" },
  { sg: "nauczyciel", nomPl: "nauczyciele", genPl: "nauczycieli", datPl: "nauczycielom", instPl: "nauczycielami", locPl: "nauczycielach" },
  { sg: "kierowca", nomPl: "kierowcy", genPl: "kierowców", datPl: "kierowcom", instPl: "kierowcami", locPl: "kierowcach" },
  { sg: "aktor", nomPl: "aktorzy", genPl: "aktorów", datPl: "aktorom", instPl: "aktorami", locPl: "aktorach" },
  { sg: "sprzedawca", nomPl: "sprzedawcy", genPl: "sprzedawców", datPl: "sprzedawcom", instPl: "sprzedawcami", locPl: "sprzedawcach" }
];

const pluralGeneralNouns = [
  { gender: "f", sg: "książka", nomPl: "książki", genPl: "książek", datPl: "książkom", instPl: "książkami", locPl: "książkach" },
  { gender: "f", sg: "wiadomość", nomPl: "wiadomości", genPl: "wiadomości", datPl: "wiadomościom", instPl: "wiadomościami", locPl: "wiadomościach" },
  { gender: "f", sg: "zupa", nomPl: "zupy", genPl: "zup", datPl: "zupom", instPl: "zupami", locPl: "zupach" },
  { gender: "f", sg: "torba", nomPl: "torby", genPl: "toreb", datPl: "torbom", instPl: "torbami", locPl: "torbach" },
  { gender: "f", sg: "szkoła", nomPl: "szkoły", genPl: "szkół", datPl: "szkołom", instPl: "szkołami", locPl: "szkołach" },
  { gender: "n", sg: "dziecko", nomPl: "dzieci", genPl: "dzieci", datPl: "dzieciom", instPl: "dziećmi", locPl: "dzieciach" },
  { gender: "n", sg: "miasto", nomPl: "miasta", genPl: "miast", datPl: "miastom", instPl: "miastami", locPl: "miastach" },
  { gender: "n", sg: "mieszkanie", nomPl: "mieszkania", genPl: "mieszkań", datPl: "mieszkaniom", instPl: "mieszkaniami", locPl: "mieszkaniach" },
  { gender: "n", sg: "zadanie", nomPl: "zadania", genPl: "zadań", datPl: "zadaniom", instPl: "zadaniami", locPl: "zadaniach" },
  { gender: "n", sg: "zdjęcie", nomPl: "zdjęcia", genPl: "zdjęć", datPl: "zdjęciom", instPl: "zdjęciami", locPl: "zdjęciach" },
  { gender: "m", sg: "dokument", nomPl: "dokumenty", genPl: "dokumentów", datPl: "dokumentom", instPl: "dokumentami", locPl: "dokumentach" },
  { gender: "m", sg: "bilet", nomPl: "bilety", genPl: "biletów", datPl: "biletom", instPl: "biletami", locPl: "biletach" },
  { gender: "m", sg: "film", nomPl: "filmy", genPl: "filmów", datPl: "filmom", instPl: "filmami", locPl: "filmach" },
  { gender: "m", sg: "komputer", nomPl: "komputery", genPl: "komputerów", datPl: "komputerom", instPl: "komputerami", locPl: "komputerach" }
];

const pluralDativeRecipientNouns = [
  { gender: "f", sg: "rodzina", nomPl: "rodziny", genPl: "rodzin", datPl: "rodzinom", instPl: "rodzinami", locPl: "rodzinach" },
  { gender: "f", sg: "grupa", nomPl: "grupy", genPl: "grup", datPl: "grupom", instPl: "grupami", locPl: "grupach" },
  { gender: "f", sg: "firma", nomPl: "firmy", genPl: "firm", datPl: "firmom", instPl: "firmami", locPl: "firmach" },
  { gender: "f", sg: "drużyna", nomPl: "drużyny", genPl: "drużyn", datPl: "drużynom", instPl: "drużynami", locPl: "drużynach" },
  { gender: "f", sg: "organizacja", nomPl: "organizacje", genPl: "organizacji", datPl: "organizacjom", instPl: "organizacjami", locPl: "organizacjach" },
  { gender: "f", sg: "szkoła", nomPl: "szkoły", genPl: "szkół", datPl: "szkołom", instPl: "szkołami", locPl: "szkołach" },
  { gender: "f", sg: "klasa", nomPl: "klasy", genPl: "klas", datPl: "klasom", instPl: "klasami", locPl: "klasach" },
  { gender: "f", sg: "redakcja", nomPl: "redakcje", genPl: "redakcji", datPl: "redakcjom", instPl: "redakcjami", locPl: "redakcjach" },
  { gender: "f", sg: "komisja", nomPl: "komisje", genPl: "komisji", datPl: "komisjom", instPl: "komisjami", locPl: "komisjach" },
  { gender: "f", sg: "ekipa", nomPl: "ekipy", genPl: "ekip", datPl: "ekipom", instPl: "ekipami", locPl: "ekipach" },
  { gender: "n", sg: "dziecko", nomPl: "dzieci", genPl: "dzieci", datPl: "dzieciom", instPl: "dziećmi", locPl: "dzieciach" },
  { gender: "n", sg: "pokolenie", nomPl: "pokolenia", genPl: "pokoleń", datPl: "pokoleniom", instPl: "pokoleniami", locPl: "pokoleniach" }
];

const pluralCaseConfigs = {
  accusativePlural: {
    caseLabel: "bierniku liczby mnogiej",
    hint: "najpierw wybierz grupę: męskoosobowy albo niemęskoosobowy",
    mascPrompts: ["Widzę", "Spotykam", "Znam", "Zapraszam", "Odwiedzam", "Słyszę"],
    nonPrompts: ["Kupuję", "Widzę", "Mam", "Czytam", "Oglądam", "Wybieram", "Pakuję", "Zamawiam", "Sprawdzam", "Niosę"]
  },
  genitivePlural: {
    caseLabel: "dopełniaczu liczby mnogiej",
    hint: "pomyśl o nie ma / szukam / potrzebuję / używam",
    mascPrompts: ["Szukam", "Potrzebuję", "Nie mam", "Nie ma", "Używam", "Słucham"],
    nonPrompts: ["Szukam", "Potrzebuję", "Nie mam", "Nie ma", "Używam", "Słucham", "Opisuję brak"]
  },
  dativePlural: {
    caseLabel: "celowniku liczby mnogiej",
    hint: "to adresat w liczbie mnogiej: komu?",
    mascPrompts: ["Pomagam", "Daję zadanie", "Wysyłam wiadomość", "Tłumaczę temat", "Pokazuję drogę", "Mówię prawdę"],
    nonPrompts: ["Pomagam", "Daję czas", "Wysyłam informacje", "Tłumaczę zasady", "Pokazuję plan", "Mówię wszystko"]
  },
  instrumentalPlural: {
    caseLabel: "narzędniku liczby mnogiej",
    hint: "pomyśl o z kim? z czym? i interesować się czym?",
    mascPrompts: ["Rozmawiam z", "Pracuję z", "Jadę z", "Spotykam się z", "Współpracuję z", "Konsultuję projekt z"],
    nonPrompts: ["Idę z", "Interesuję się", "Zajmuję się", "Jestem między", "Pracuję nad", "Wyruszam z"]
  },
  locativePlural: {
    caseLabel: "miejscowniku liczby mnogiej",
    hint: "sprawdź, czy chodzi o: o kim? o czym? gdzie?",
    mascPrompts: ["Myślę o", "Rozmawiam o", "Czytam o", "Mówię o", "Piszę o", "Słyszę o"],
    nonPrompts: ["Myślę o", "Rozmawiam o", "Czytam o", "Mówię o", "Jestem na", "Byłem w", "Uczę się na", "Pracuję w"]
  }
};

function buildPluralPhraseEntries(caseKey) {
  const cfg = pluralCaseConfigs[caseKey];
  const entries = [];

  pluralMascPersonalNouns.forEach((noun, nounIndex) => {
    pluralAdjectivePatterns.forEach((adj, adjIndex) => {
      const prompt = cfg.mascPrompts[(nounIndex + adjIndex) % cfg.mascPrompts.length];
      const singular = `${adj.m} ${noun.sg}`;
      const base = `${adj.nomMasc} ${noun.nomPl}`;
      const nounTarget = caseKey === "dativePlural" ? noun.datPl : caseKey === "instrumentalPlural" ? noun.instPl : caseKey === "locativePlural" ? noun.locPl : noun.genPl;
      const targetAdj = caseKey === "dativePlural" ? adj.dat : caseKey === "instrumentalPlural" ? adj.inst : adj.genLoc;
      const target = `${targetAdj} ${nounTarget}`;
      const wrong = base;
      entries.push({ singular, base, target, wrong, prompt });
    });
  });

  const nonSource = caseKey === "dativePlural" ? pluralDativeRecipientNouns : pluralGeneralNouns;
  nonSource.forEach((noun, nounIndex) => {
    pluralAdjectivePatterns.forEach((adj, adjIndex) => {
      const prompt = cfg.nonPrompts[(nounIndex + adjIndex) % cfg.nonPrompts.length];
      const singularAdj = noun.gender === "f" ? adj.f : noun.gender === "n" ? adj.n : adj.m;
      const singular = `${singularAdj} ${noun.sg}`;
      const base = `${adj.nomNonMasc} ${noun.nomPl}`;
      const nounTarget = caseKey === "accusativePlural" ? noun.nomPl : caseKey === "dativePlural" ? noun.datPl : caseKey === "instrumentalPlural" ? noun.instPl : caseKey === "locativePlural" ? noun.locPl : noun.genPl;
      const targetAdj = caseKey === "accusativePlural" ? adj.nomNonMasc : caseKey === "dativePlural" ? adj.dat : caseKey === "instrumentalPlural" ? adj.inst : adj.genLoc;
      const target = `${targetAdj} ${nounTarget}`;
      const wrong = caseKey === "accusativePlural" ? `${adj.genLoc} ${noun.genPl}` : base;
      entries.push({ singular, base, target, wrong, prompt });
    });
  });
...
const pluralCasePoolCache = {};

function getPluralCasePool(caseKey) {
  if (!pluralCasePoolCache[caseKey]) {
    pluralCasePoolCache[caseKey] = buildPluralCasePool(buildPluralPhraseEntries(caseKey), pluralCaseConfigs[caseKey]);
  }
  return pluralCasePoolCache[caseKey];
}

function getPluralCaseNuancePool(caseKey) {
  return takeExerciseSlot(getPluralCasePool(caseKey), EXERCISE_MAX, 24);
}

function getPluralCaseStepPool(caseKey) {
  return takeExerciseSlot(getPluralCasePool(caseKey), EXERCISE_MAX + 24, 24);
}

function workbook(title, lines, blanks, explanation = "") {
  return cloze(title, title, lines, blanks.map((answers) => ({ answers: Array.isArray(answers) ? answers : [answers] })), explanation);
}

function genCaseWorkbook(topic) {
  const sets = {
    accusative: [
      workbook(
        "Wpisz słowa z nawiasów w bierniku",
        [
          "Poproszę [1] (kawa). A ja poproszę [2] (sok). Jaki? Pomarańczowy.",
          "Czy znasz [3] (Adam) i [4] (Ewa)? Tak, znam.",
          "Czy znacie [5] (restauracja) „Fenix”? Tak, znamy.",
          "Czy pani zna [6] (pub) „Lolek”?",
          "Czy pan lubi [7] (piwo)? Tak, lubię. A [8] (wino)? Też lubię.",
          "Bardzo lubię [9] (Marek) i [10] (Edyta)."
        ],
        [
          "kawę",
          "sok",
        ...
        "Ćwicz biernik w gotowych zdaniach z ludźmi, jedzeniem i miejscami."
      ),
      workbook(
        "Biernik: restauracja, urząd, zakupy",
        [
          "W urzędzie pokazuję [1] (ważny dokument) i proszę o pomoc.",
          "Na obiad gotuję [2] (smaczna zupa), a potem robię [3] (krótka sałatka).",
          "Wieczorem oglądam [4] (ciekawy serial), ale rano czytam [5] (interesująca gazeta).",
          "Na uczelni spotykam [6] (nowy wykładowca), a po zajęciach odwiedzam [7] (miła koleżanka).",
          "W aptece biorę [8] (tańszy lek), a w sklepie kupuję [9] (świeży chleb)."
        ],
        [
          "ważny dokument",
          "smaczną zupę",
        ...
        "Смотри на глагол и сразу задавай вопрос `kogo? co?`."
      ),
      workbook(
        "Biernik: liczba mnoga w zdaniach",
        [
          "W weekend zapraszam [1] (nowi studenci), a wieczorem spotykam [2] (mili sąsiedzi).",
          "W sklepie kupuję [3] (tanie bilety) i [4] (świeże warzywa).",
          "Na biurku mam [5] (ważne dokumenty), ale dziś czytam tylko [6] (ciekawe artykuły).",
          "Na zdjęciu widzę [7] (małe dzieci), a w sali słyszę [8] (ważne pytania)."
        ],
        [
          "nowych studentów",
          "miłych sąsiadów",
        ...
        "Отдельно чувствуем męskoosobowy и niemęskoosobowy."
      )
    ],
    genitive: [
      workbook(
        "Wpisz słowa z nawiasów w dopełniaczu",
        [
          "Dziś nie ma [1] (wolny termin), więc musimy poczekać.",
          "W pracy potrzebuję [2] (nowy dokument) i [3] (spokojne miejsce).",
          "Po południu szukam [4] (tanie mieszkanie), a wieczorem używam [5] (duży nóż).",
          "Na kursie uczę się [6] (polska wymowa), a w domu słucham [7] (ciekawy podcast).",
          "W lodówce nie ma [8] (świeży chleb), ale jest trochę [9] (sok)."
        ],
        [
          "wolnego terminu",
          "nowego dokumentu",
        ...
        "Здесь работают: `nie ma`, `potrzebuję`, `szukam`, `używam`, `uczę się`, `słucham`."
      ),
      workbook(
        "Dopełniacz: liczba mnoga w kontekście",
        [
          "W tej sali nie ma [1] (wolne miejsca).",
          "Do egzaminu potrzebuję [2] (nowe dokumenty) i [3] (dobre notatki).",
          "W sklepie szukam [4] (świeże warzywa), ale nie widzę [5] (tanie bilety).",
          "Do sałatki używam [6] (ostre noże), a do pracy potrzebuję [7] (wygodne buty)."
        ],
        [
          "wolnych miejsc",
          "nowych dokumentów",
        ...
        "Во множественном числе особенно следи за прилагательным `-ych / -ich`."
      )
    ],
    dative: [
      workbook(
        "Wpisz słowa z nawiasów w celowniku",
        [
          "Codziennie pomagam [1] (moja siostra), a czasem też [2] (starszy sąsiad).",
          "Wysyłam wiadomość [3] (nowy student) i tłumaczę zadanie [4] (małe dziecko).",
          "Na spotkaniu mówię prawdę [5] (dobry kolega), a dokument daję [6] (kierownik).",
          "Po zajęciach odpowiadam [7] (nauczyciel) i pokazuję drogę [8] (starsza pani)."
        ],
        [
          "mojej siostrze",
          "starszemu sąsiadowi",
        ...
        "Сначала находи адресата: `komu?`."
      ),
      workbook(
        "Celownik: liczba mnoga",
        [
          "Na kursie tłumaczę temat [1] (nowi studenci).",
          "W pracy wysyłam informacje [2] (mili klienci).",
          "Po południu pomagam [3] (moi rodzice), a wieczorem mówię wszystko [4] (nowe grupy).",
          "Na końcu daję zadanie [5] (wszyscy uczestnicy) i pokazuję drogę [6] (zagraniczni turyści)."
        ],
        [
          "nowym studentom",
          "miłym klientom",
        ...
        "Во множественном числе ищи адресата и очень часто выходишь на `-om`."
      )
    ],
    instrumental: [
      workbook(
        "Wpisz słowa z nawiasów w narzędniku",
        [
          "Jestem [1] (nowy pracownik), ale dziś jadę też z [2] (miły kolega).",
          "Rozmawiam z [3] (dobry nauczyciel), a potem spotykam się z [4] (starsza pani).",
          "Interesuję się [5] (polska kultura) i zajmuję się [6] (ważny projekt).",
          "Po pracy idę z [7] (małe dziecko) i wracam z [8] (rodzina)."
        ],
        [
          "nowym pracownikiem",
          "miłym kolegą",
        ...
        "Смотри на конструкции `z kim?`, `jestem kim?`, `interesuję się czym?`."
      ),
      workbook(
        "Narzędnik: liczba mnoga",
        [
          "Pracuję z [1] (mili ludzie), ale dziś rozmawiam też z [2] (nowi klienci).",
          "Interesuję się [3] (obce języki) i zajmuję się [4] (ważne projekty).",
          "W weekend jadę z [5] (moi rodzice), a po południu idę z [6] (małe dzieci)."
        ],
        [
          "miłymi ludźmi",
          "nowymi klientami",
        ...
        "Во множественном числе доводи до автоматизма `-ami / -mi`."
      )
    ],
    locative: [
      workbook(
        "Wpisz słowa z nawiasów w miejscowniku",
        [
          "Mieszkam w [1] (spokojna dzielnica), ale pracuję w [2] (nowe biuro).",
          "Na kursie rozmawiamy o [3] (ważny egzamin) i myślimy o [4] (krótki urlop).",
          "Po pracy jestem na [5] (długie spotkanie), a wieczorem czytam o [6] (nowa praca).",
          "Rano mówię o [7] (trudny problem), a potem jestem po [8] (ciężki dzień)."
        ],
        [
          "spokojnej dzielnicy",
          "nowym biurze",
        ...
        "Сначала понимай вопрос: `gdzie?` или `o czym?`."
      ),
      workbook(
        "Miejscownik: liczba mnoga",
        [
          "Na zajęciach mówimy o [1] (ważne plany) i [2] (ciekawe książki).",
          "W weekend byłem w [3] (duże sklepy), ale czytałem też o [4] (polskie miasta).",
          "Po południu jestem na [5] (trudne zajęcia), a wieczorem myślę o [6] (nowe zasady)."
        ],
        [
          "ważnych planach",
          "ciekawych książkach",
        ...
        "Во множественном числе часто узнаёшь местный по `-ach`."
      )
    ]
  };
  return sets[topic] || [];
...
function buildWorkbookLine(question, blankNumber) {
  const raw = String(question || "").trim();
  const withoutPrefix = raw
    .replace(/^Uzupełnij formę w [^:]+:\s*/i, "")
    .replace(/^Uzupełnij formę:\s*/i, "")
    .replace(/^Uzupełnij:\s*/i, "")
    .trim();
  const replaced = withoutPrefix.replace(/\(([^()]*)\)\.?\s*$/i, `[${blankNumber}] ($1).`);
  return replaced !== withoutPrefix ? replaced : `${withoutPrefix} [${blankNumber}].`;
}

function genWorkbookDrills(topic) {
  const toWorkbookItems = (title, sourceItems, explanation, linesPerPage = 6, pages = 4) => {
    const filtered = uniqueExerciseItems(sourceItems).filter((item) => item?.type === "input" && /\([^()]+\)/.test(String(item.q || "")));
    if (!filtered.length) return [];
    const picked = takeExerciseSlot(filtered, 0, Math.min(filtered.length, linesPerPage * pages));
    const workbookItems = [];
    for (let page = 0; page < pages; page += 1) {
      const chunk = picked.slice(page * linesPerPage, (page + 1) * linesPerPage);
      if (!chunk.length) continue;
      const lines = chunk.map((item, index) => buildWorkbookLine(item.q, index + 1));
      const blanks = chunk.map((item) => item.a || []);
      workbookItems.push(workbook(`${title} ${page + 1}`, lines, blanks, explanation));
    }
  ...
  const map = {
    accusative: [
      ...genCaseWorkbook("accusative"),
      ...toWorkbookItems("Biernik: zeszyt ćwiczeń", [...genAccusativeForms(), ...genAccusativeAdjectives(), ...genAccusativeVariety()], "Пиши форму прямо в готовом предложении.")
    ],
    accusativePlural: [
      ...toWorkbookItems("Biernik liczby mnogiej: zeszyt ćwiczeń", [...genAccusativePlural(), ...getPluralCaseNuancePool("accusativePlural")], "Отдельно держи в голове męskoosobowy и niemęskoosobowy.")
    ],
    genitive: [
      ...genCaseWorkbook("genitive"),
      ...toWorkbookItems("Dopełniacz: zeszyt ćwiczeń", [...genGenitive(), ...genGenitiveVariety()], "Здесь важны связки `nie ma`, `szukam`, `potrzebuję`, `używam`.")
    ],
    genitivePlural: [
      ...toWorkbookItems("Dopełniacz liczby mnogiej: zeszyt ćwiczeń", [...genGenitivePlural(), ...getPluralCaseNuancePool("genitivePlural")], "Во множественном числе следи за `-ów / -i / -y / zero ending`.")
    ],
    dative: [
      ...genCaseWorkbook("dative"),
      ...toWorkbookItems("Celownik: zeszyt ćwiczeń", [...genDative(), ...genDativeVariety()], "Сначала находи адресата: `komu?`.")
    ],
    dativePlural: [
      ...toWorkbookItems("Celownik liczby mnogiej: zeszyt ćwiczeń", [...genDativePlural(), ...getPluralCaseNuancePool("dativePlural")], "Во множественном числе очень часто увидишь `-om`.")
    ],
    instrumental: [
      ...genCaseWorkbook("instrumental"),
      ...toWorkbookItems("Narzędnik: zeszyt ćwiczeń", [...genInstrumental(), ...genInstrumental()], "Держи конструкции `z kim?`, `jestem kim?`, `interesuję się czym?`.")
    ],
    instrumentalPlural: [
      ...toWorkbookItems("Narzędnik liczby mnogiej: zeszyt ćwiczeń", [...genInstrumentalPlural(), ...getPluralCaseNuancePool("instrumentalPlural")], "Доводи до автоматизма `-ami / -mi`.")
    ],
    locative: [
      ...genCaseWorkbook("locative"),
      ...toWorkbookItems("Miejscownik: zeszyt ćwiczeń", [...genLocative(), ...genLocativeVariety()], "Сначала решай: `gdzie?` или `o czym?`.")
    ],
    locativePlural: [
      ...toWorkbookItems("Miejscownik liczby mnogiej: zeszyt ćwiczeń", [...genLocativePlural(), ...getPluralCaseNuancePool("locativePlural")], "Во множественном числе часто узнаёшь тему по `-ach`.")
    ],
    verbsPresent: [
      ...toWorkbookItems("Czas teraźniejszy: zeszyt ćwiczeń", [...genPresentConjugationDrills(), ...genPresent(), ...genPresentVariety()], "Сначала определяй лицо и модель глагола.")
    ],
    verbsPast: [
      ...toWorkbookItems("Czas przeszły: zeszyt ćwiczeń", genPast(), "Следи за лицом и родом.")
    ],
    verbsFuture: [
      ...toWorkbookItems("Czas przyszły: zeszyt ćwiczeń", [...genFuture(), ...genFutureWithTime()], "Выбирай между процессом и результатом.")
    ],
    prepositions: [
      ...toWorkbookItems("Przyimki: zeszyt ćwiczeń", genPrepositions(), "Сначала понимай вопрос: gdzie, dokąd, skąd, z kim, o czym.")
    ],
    pronouns: [
      ...toWorkbookItems("Zaimki: zeszyt ćwiczeń", genPronouns(), "Подставляй местоимение по вопросу `komu?`, `kogo?`, `czego?`.")
    ],
    reflexiveSie: [
      ...toWorkbookItems("Czasowniki z się: zeszyt ćwiczeń", genReflexiveSie(), "Учи целую конструкцию, а не отдельное `się`.")
    ]
  };
  return map[topic] || [];
}
...
const ruleTables = {
  nominative: [
    { title: "Mianownik — tabela końcówek pojedyncza", headers: ["część mowy", "rodzaj męski", "rodzaj żeński", "rodzaj nijaki"], rows: [["przymiotnik", "-y / -i", "-a", "-e"], ["rzeczownik", "forma podstawowa", "forma podstawowa", "forma podstawowa"], ["przykład", "dobry lekarz / nowy dokument", "dobra kawa / miła koleżanka", "małe dziecko / nowe mieszkanie"]] },
    { title: "Mianownik — kto? co?", headers: ["rodzaj", "model", "przykład"], rows: [["męski", "kto? co? forma podstawowa", "to jest lekarz / to jest dokument"], ["żeński", "kto? co? forma podstawowa", "to jest kobieta / to jest kawa"], ["nijaki", "kto? co? forma podstawowa", "to jest dziecko / to jest mieszkanie"], ["po `to jest`", "mianownik", "to jest mój brat"], ["po `to są`", "mianownik liczby mnogiej", "to są moi rodzice / to są nowe książki"]] }
  ],
  pluralNominative: [
  ...
  accusativePlural: [
    { title: "Biernik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "biernik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrych / nowych / polskich", "widzę dobrych studentów"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobre / nowe / polskie", "mam nowe dokumenty"]] },
    { title: "Biernik liczby mnogiej — rzeczownik", headers: ["grupa", "mianownik", "biernik", "przykład"], rows: [["męskoosobowy", "studenci, koledzy, lekarze", "studentów, kolegów, lekarzy", "znam polskich studentów"], ["niemęskoosobowy", "książki, dokumenty, dzieci", "książki, dokumenty, dzieci", "widzę ciekawe książki"]] },
    { title: "Biernik liczby mnogiej", headers: ["grupa", "mianownik", "biernik", "przykład"], rows: [["męskoosobowy", "nowi koledzy", "nowych kolegów", "widzę nowych kolegów"], ["męskoosobowy", "polscy studenci", "polskich studentów", "znam polskich studentów"], ["męskoosobowy", "dobrzy lekarze", "dobrych lekarzy", "spotykam dobrych lekarzy"], ["niemęskoosobowy", "nowe dokumenty", "nowe dokumenty", "mam nowe dokumenty"], ["niemęskoosobowy", "małe dzieci", "małe dzieci", "widzę małe dzieci"], ["niemęskoosobowy", "tanie bilety", "tanie bilety", "kupuję tanie bilety"]] }
  ],
  accusative: [
    { title: "Biernik — tabela końcówek pojedyncza", headers: ["część mowy", "rodzaj męski żywotny", "rodzaj męski nieżywotny", "rodzaj żeński", "rodzaj nijaki"], rows: [["przymiotnik", "-ego", "= mianownik", "-ą", "= mianownik"], ["rzeczownik", "-a / -ę", "= mianownik", "-ę", "= mianownik"], ["przykład", "widzę dobrego lekarza", "mam dobry obiad", "mam ładną koleżankę", "jem smaczne śniadanie"]] },
    { title: "Biernik — co dzieje się z rzeczownikiem", headers: ["rodzaj", "mianownik", "biernik", "najczęstszy model"], rows: [["męski żywotny", "lekarz / student / pies", "lekarza / studenta / psa", "często `-a / -ę`"], ["męski nieżywotny", "telefon / obiad / bilet", "telefon / obiad / bilet", "często bez zmiany"], ["żeński", "kawa / książka / praca", "kawę / książkę / pracę", "`-a -> -ę`"], ["nijaki", "dziecko / mieszkanie / śniadanie", "dziecko / mieszkanie / śniadanie", "często bez zmiany"]] },
    { title: "Biernik — końcówki", headers: ["rodzaj", "mianownik", "biernik", "przykład"], rows: [["męski żywotny", "dobry lekarz", "dobrego lekarza", "Widzę dobrego lekarza"], ["męski nieżywotny", "nowy telefon", "nowy telefon", "Mam nowy telefon"], ["żeński", "dobra kawa", "dobrą kawę", "Piję dobrą kawę"], ["nijaki", "małe dziecko", "małe dziecko", "Widzę małe dziecko"], ["męskoos. mn.", "dobrzy studenci", "dobrych studentów", "Znam dobrych studentów"], ["niemęskoos. mn.", "dobre książki", "dobre książki", "Kupuję dobre książki"]] },
    { title: "Biernik liczby mnogiej — osobno", headers: ["grupa", "mianownik", "biernik", "model"], rows: [["męskoosobowy", "nowi koledzy", "nowych kolegów", "widzę nowych kolegów"], ["męskoosobowy", "polscy studenci", "polskich studentów", "znam polskich studentów"], ["niemęskoosobowy", "nowe dokumenty", "nowe dokumenty", "mam nowe dokumenty"], ["niemęskoosobowy", "małe dzieci", "małe dzieci", "widzę małe dzieci"], ["niemęskoosobowy", "tanie bilety", "tanie bilety", "kupuję tanie bilety"], ["niemęskoosobowy", "gorące zupy", "gorące zupy", "zamawiam gorące zupy"]] }],
  genitivePlural: [
    { title: "Dopełniacz liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "dopełniacz", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrych / nowych / polskich", "szukam dobrych studentów"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrych / nowych / polskich", "nie ma dobrych książek"]] },
    { title: "Dopełniacz liczby mnogiej — rzeczownik", headers: ["typ", "najczęstsza końcówka", "model", "przykład"], rows: [["męskoosobowy", "-ów / -i / -y", "studentów, lekarzy, kolegów", "szukam nowych studentów"], ["żeńskie", "zero / -y / -i", "kobiet, książek, restauracji", "nie ma dobrych książek"], ["nijakie", "zero / -i / -ów", "miejsc, mieszkań, pytań", "potrzebuję wolnych miejsc"], ["męskie nieosobowe", "-ów / zero / -i", "sklepów, butów, noży", "używam wygodnych butów"]] },
    { title: "Dopełniacz liczby mnogiej", headers: ["mianownik", "dopełniacz", "przykład"], rows: [["nowe dokumenty", "nowych dokumentów", "potrzebuję nowych dokumentów"], ["dobre książki", "dobrych książek", "szukam dobrych książek"], ["małe sklepy", "małych sklepów", "nie ma małych sklepów"], ["wygodne buty", "wygodnych butów", "potrzebuję wygodnych butów"], ["świeże warzywa", "świeżych warzyw", "używam świeżych warzyw"], ["wolne miejsca", "wolnych miejsc", "nie ma wolnych miejsc"]] }
  ],
  genitive: [
    { title: "Dopełniacz — tabela końcówek pojedyncza", headers: ["część mowy", "rodzaj męski", "rodzaj żeński", "rodzaj nijaki"], rows: [["przymiotnik", "-ego", "-ej", "-ego"], ["rzeczownik", "często -a / -u", "często -y / -i", "często -a"], ["przykład", "dobrego lekarza / ważnego dokumentu", "dobrej kawy / nowej pracy", "małego dziecka / nowego mieszkania"]] },
    { title: "Dopełniacz — końcówki", headers: ["typ", "mianownik", "dopełniacz", "użycie"], rows: [["męski", "czas", "czasu", "nie mam czasu"], ["męski", "student", "studenta", "nie ma studenta"], ["żeński", "kawa", "kawy", "nie ma kawy"], ["żeński", "praca", "pracy", "szukam pracy"], ["nijaki", "dziecko", "dziecka", "nie ma dziecka"], ["mnoga", "ludzie", "ludzi", "dużo ludzi"]] },
    { title: "Dopełniacz — прилагательные и количество", headers: ["mianownik", "dopełniacz", "przykład"], rows: [["dobry lekarz", "dobrego lekarza", "nie ma dobrego lekarza"], ["ważny dokument", "ważnego dokumentu", "szukam ważnego dokumentu"], ["dobra kawa", "dobrej kawy", "nie ma dobrej kawy"], ["małe dziecko", "małego dziecka", "nie ma małego dziecka"], ["dużo", "+ dopełniacz", "dużo ludzi / dużo pracy"], ["mało / trochę", "+ dopełniacz", "mało czasu / trochę wody"]] },
    { title: "Dopełniacz liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "dopełniacz", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrych / nowych / polskich", "szukam dobrych studentów"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrych / nowych / polskich", "nie ma dobrych książek"]] },
    { title: "Dopełniacz liczby mnogiej — rzeczownik", headers: ["typ", "najczęstsza końcówka", "model", "przykład"], rows: [["męskoosobowy", "-ów / -i / -y", "studentów, lekarzy, kolegów", "szukam nowych studentów"], ["żeńskie", "zero / -y / -i", "kobiet, książek, restauracji", "nie ma dobrych książek"], ["nijakie", "zero / -i / -ów", "miejsc, mieszkań, pytań", "potrzebuję wolnych miejsc"], ["męskie nieosobowe", "-ów / zero / -i", "sklepów, butów, noży", "używam wygodnych butów"]] },
    { title: "Dopełniacz liczby mnogiej — gotowe modele", headers: ["mianownik", "dopełniacz", "przykład"], rows: [["nowe dokumenty", "nowych dokumentów", "potrzebuję nowych dokumentów"], ["dobre książki", "dobrych książek", "szukam dobrych książek"], ["małe sklepy", "małych sklepów", "nie ma małych sklepów"], ["wygodne buty", "wygodnych butów", "potrzebuję wygodnych butów"], ["świeże warzywa", "świeżych warzyw", "używam świeżych warzyw"], ["wolne miejsca", "wolnych miejsc", "nie ma wolnych miejsc"]] }
  ],
  dativePlural: [
    { title: "Celownik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "celownik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrym / nowym / polskim", "pomagam dobrym studentom"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrym / nowym / polskim", "daję nowe zadania małym dzieciom"]] },
    { title: "Celownik liczby mnogiej — rzeczownik", headers: ["typ", "mianownik", "celownik", "przykład"], rows: [["męskoosobowy", "studenci, koledzy, klienci", "studentom, kolegom, klientom", "tłumaczę klientom"], ["żeńskie", "kobiety, grupy, rodziny", "kobietom, grupom, rodzinom", "mówię kobietom"], ["nijakie", "dzieci, mieszkania, pytania", "dzieciom, mieszkaniom, pytaniom", "daję dzieciom czas"]] },
    { title: "Celownik liczby mnogiej", headers: ["mianownik", "celownik", "przykład"], rows: [["moi rodzice", "moim rodzicom", "pomagam moim rodzicom"], ["nowi pracownicy", "nowym pracownikom", "tłumaczę pracownikom"], ["mili klienci", "miłym klientom", "wysyłam klientom wiadomość"], ["małe dzieci", "małym dzieciom", "daję dzieciom czas"], ["wszyscy uczestnicy", "wszystkim uczestnikom", "pokazuję uczestnikom drogę"], ["nowe grupy", "nowym grupom", "mówię nowym grupom wszystko"]] }
  ],
  dative: [
    { title: "Celownik — tabela końcówek pojedyncza", headers: ["część mowy", "rodzaj męski", "rodzaj żeński", "rodzaj nijaki"], rows: [["przymiotnik", "-emu", "-ej", "-emu"], ["rzeczownik", "często -owi / -u", "często -e / -ie / -y", "często -u"], ["przykład", "dobremu studentowi / bratu", "miłej kobiecie / mamie", "małemu dziecku"]] },
    { title: "Celownik — komu? czemu?", headers: ["mianownik", "celownik", "przykład"], rows: [["ja", "mi", "daje mi"], ["ty", "ci", "pomagam ci"], ["on", "mu", "mówię mu"], ["student", "studentowi", "pomagam studentowi"], ["kolega", "koledze", "mówię koledze"], ["kobieta", "kobiecie", "pomagam kobiecie"], ["dziecko", "dziecku", "daję dziecku"]] },
    { title: "Celownik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "celownik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrym / nowym / polskim", "pomagam dobrym studentom"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrym / nowym / polskim", "daję nowe zadania małym dzieciom"]] },
    { title: "Celownik liczby mnogiej — rzeczownik", headers: ["typ", "mianownik", "celownik", "przykład"], rows: [["męskoosobowy", "studenci, koledzy, klienci", "studentom, kolegom, klientom", "tłumaczę klientom"], ["żeńskie", "kobiety, grupy, rodziny", "kobietom, grupom, rodzinom", "mówię kobietom"], ["nijakie", "dzieci, mieszkania, pytania", "dzieciom, mieszkaniom, pytaniom", "daję dzieciom czas"]] },
    { title: "Celownik liczby mnogiej — gotowe modele", headers: ["mianownik", "celownik", "model"], rows: [["moi rodzice", "moim rodzicom", "pomagam moim rodzicom"], ["nowi pracownicy", "nowym pracownikom", "tłumaczę pracownikom"], ["mili klienci", "miłym klientom", "wysyłam klientom wiadomość"], ["małe dzieci", "małym dzieciom", "daję dzieciom czas"], ["wszyscy uczestnicy", "wszystkim uczestnikom", "pokazuję uczestnikom drogę"], ["nowe grupy", "nowym grupom", "mówię nowym grupom wszystko"]] }
  ],
  instrumentalPlural: [
    { title: "Narzędnik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "narzędnik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrymi / nowymi / polskimi", "pracuję z dobrymi studentami"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrymi / nowymi / polskimi", "interesuję się nowymi książkami"]] },
    { title: "Narzędnik liczby mnogiej — rzeczownik", headers: ["typ", "mianownik", "narzędnik", "przykład"], rows: [["męskoosobowy", "studenci, klienci, koledzy", "studentami, klientami, kolegami", "rozmawiam z klientami"], ["żeńskie", "kobiety, rodziny, grupy", "kobietami, rodzinami, grupami", "jadę z rodzinami"], ["nijakie", "dzieci, pytania, mieszkania", "dziećmi, pytaniami, mieszkaniami", "idę z dziećmi"], ["nieregularne", "ludzie, przyjaciele", "ludźmi, przyjaciółmi", "pracuję z ludźmi"]] },
    { title: "Narzędnik liczby mnogiej", headers: ["mianownik", "narzędnik", "przykład"], rows: [["mili ludzie", "miłymi ludźmi", "pracuję z miłymi ludźmi"], ["nowi klienci", "nowymi klientami", "rozmawiam z nowymi klientami"], ["obce języki", "obcymi językami", "interesuję się obcymi językami"], ["ważne projekty", "ważnymi projektami", "zajmuję się ważnymi projektami"], ["nowoczesne technologie", "nowoczesnymi technologiami", "interesuję się nowoczesnymi technologiami"], ["małe dzieci", "małymi dziećmi", "idę z małymi dziećmi"]] }
  ],
  instrumental: [
    { title: "Narzędnik — tabela końcówek pojedyncza", headers: ["część mowy", "rodzaj męski", "rodzaj żeński", "rodzaj nijaki"], rows: [["przymiotnik", "-ym / -im", "-ą", "-ym / -im"], ["rzeczownik", "-em", "-ą", "-em"], ["przykład", "dobrym lekarzem", "miłą kobietą", "małym dzieckiem"]] },
    { title: "Narzędnik — z kim? z czym?", headers: ["mianownik", "narzędnik", "przykład"], rows: [["programista", "programistą", "jestem programistą"], ["student", "studentem", "jestem studentem"], ["lekarz", "lekarzem", "jestem lekarzem"], ["kolega", "kolegą", "z kolegą"], ["rodzina", "rodziną", "z rodziną"], ["dziecko", "dzieckiem", "z dzieckiem"], ["ludzie", "ludźmi", "z ludźmi"]] },
    { title: "Narzędnik — типовые окончания", headers: ["typ", "końcówka", "przykład"], rows: [["męski", "-em", "studentem, dokumentem"], ["nijaki", "-em", "dzieckiem, mieszkaniem"], ["żeński", "-ą", "kobietą, rodziną, książką"], ["mnoga", "-ami / -mi", "studentami, samochodami, ludźmi"], ["po `z`", "narzędnik", "z kolegą, z rodziną"], ["po `jestem`", "narzędnik", "jestem nauczycielem"]] },
    { title: "Narzędnik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "narzędnik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrymi / nowymi / polskimi", "pracuję z dobrymi studentami"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrymi / nowymi / polskimi", "interesuję się nowymi książkami"]] },
    { title: "Narzędnik liczby mnogiej — rzeczownik", headers: ["typ", "mianownik", "narzędnik", "przykład"], rows: [["męskoosobowy", "studenci, klienci, koledzy", "studentami, klientami, kolegami", "rozmawiam z klientami"], ["żeńskie", "kobiety, rodziny, grupy", "kobietami, rodzinami, grupami", "jadę z rodzinami"], ["nijakie", "dzieci, pytania, mieszkania", "dziećmi, pytaniami, mieszkaniami", "idę z dziećmi"], ["nieregularne", "ludzie, przyjaciele", "ludźmi, przyjaciółmi", "pracuję z ludźmi"]] },
    { title: "Narzędnik liczby mnogiej — gotowe modele", headers: ["mianownik", "narzędnik", "przykład"], rows: [["mili ludzie", "miłymi ludźmi", "pracuję z miłymi ludźmi"], ["nowi klienci", "nowymi klientami", "rozmawiam z nowymi klientami"], ["obce języki", "obcymi językami", "interesuję się obcymi językami"], ["ważne projekty", "ważnymi projektami", "zajmuję się ważnymi projektami"], ["nowoczesne technologie", "nowoczesnymi technologiami", "interesuję się nowoczesnymi technologiami"], ["małe dzieci", "małymi dziećmi", "idę z małymi dziećmi"]] }
  ],
  locativePlural: [
    { title: "Miejscownik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "miejscownik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrych / nowych / polskich", "mówię o dobrych studentach"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrych / nowych / polskich", "czytam o nowych książkach"]] },
    { title: "Miejscownik liczby mnogiej — rzeczownik", headers: ["typ", "mianownik", "miejscownik", "przykład"], rows: [["męskoosobowy", "studenci, pracownicy, koledzy", "studentach, pracownikach, kolegach", "rozmawiam o pracownikach"], ["żeńskie", "książki, grupy, rodziny", "książkach, grupach, rodzinach", "myślę o książkach"], ["nijakie", "miejsca, pytania, spotkania", "miejscach, pytaniach, spotkaniach", "jestem na spotkaniach"], ["częsty wzór", "sklepy, miasta, plany", "sklepach, miastach, planach", "w sklepach / o planach"]] },
    { title: "Miejscownik liczby mnogiej", headers: ["mianownik", "miejscownik", "przykład"], rows: [["ważne egzaminy", "ważnych egzaminach", "myślę o ważnych egzaminach"], ["nowi pracownicy", "nowych pracownikach", "rozmawiam o nowych pracownikach"], ["polskie miasta", "polskich miastach", "czytam o polskich miastach"], ["ciekawe książki", "ciekawych książkach", "mówię o ciekawych książkach"], ["ważne spotkania", "ważnych spotkaniach", "jestem na ważnych spotkaniach"], ["dobre pomysły", "dobrych pomysłach", "rozmawiam o dobrych pomysłach"]] }
  ],
  locative: [
    { title: "Miejscownik — tabela końcówek pojedyncza", headers: ["część mowy", "rodzaj męski", "rodzaj żeński", "rodzaj nijaki"], rows: [["przymiotnik", "-ym / -im", "-ej", "-ym / -im"], ["rzeczownik", "często -e / -ie / -u", "często -e / -ie / -y", "często -e / -u / -iu"], ["przykład", "o dobrym kursie / w domu", "o miłej rodzinie / w pracy", "o małym dziecku / w mieszkaniu"]] },
    { title: "Miejscownik — o kim? o czym?", headers: ["mianownik", "miejscownik", "przykład"], rows: [["Polska", "Polsce", "w Polsce"], ["praca", "pracy", "w pracy"], ["sklep", "sklepie", "w sklepie"], ["dom", "domu", "w domu"], ["kurs", "kursie", "na kursie"], ["rodzina", "rodzinie", "o rodzinie"], ["dziecko", "dziecku", "o dziecku"]] },
    { title: "Miejscownik — где или куда?", headers: ["sytuacja", "pytanie", "przykład"], rows: [["miejsce", "gdzie?", "jestem na kursie / w sklepie"], ["ruch", "dokąd?", "idę na kurs / do sklepu"], ["temat rozmowy", "o czym?", "mówię o pracy"], ["po czym?", "po + miejscownik", "po pracy"], ["przy czym?", "przy + miejscownik", "przy stole"], ["w / na", "сначала смысл", "na kursie, ale na kurs"]] },
    { title: "Miejscownik liczby mnogiej — przymiotnik", headers: ["grupa", "mianownik", "miejscownik", "przykład"], rows: [["męskoosobowy", "dobrzy / nowi / polscy", "dobrych / nowych / polskich", "mówię o dobrych studentach"], ["niemęskoosobowy", "dobre / nowe / polskie", "dobrych / nowych / polskich", "czytam o nowych książkach"]] },
    { title: "Miejscownik liczby mnogiej — rzeczownik", headers: ["typ", "mianownik", "miejscownik", "przykład"], rows: [["męskoosobowy", "studenci, pracownicy, koledzy", "studentach, pracownikach, kolegach", "rozmawiam o pracownikach"], ["żeńskie", "książki, grupy, rodziny", "książkach, grupach, rodzinach", "myślę o książkach"], ["nijakie", "miejsca, pytania, spotkania", "miejscach, pytaniach, spotkaniach", "jestem na spotkaniach"], ["częsty wzór", "sklepy, miasta, plany", "sklepach, miastach, planach", "w sklepach / o planach"]] },
    { title: "Miejscownik liczby mnogiej — gotowe modele", headers: ["mianownik", "miejscownik", "przykład"], rows: [["ważne egzaminy", "ważnych egzaminach", "myślę o ważnych egzaminach"], ["nowi pracownicy", "nowych pracownikach", "rozmawiam o nowych pracownikach"], ["polskie miasta", "polskich miastach", "czytam o polskich miastach"], ["ciekawe książki", "ciekawych książkach", "mówię o ciekawych książkach"], ["ważne spotkania", "ważnych spotkaniach", "jestem na ważnych spotkaniach"], ["dobre pomysły", "dobrych pomysłach", "rozmawiam o dobrych pomysłach"]] }
  ],
  verbsPresent: [
    { title: "Czas teraźniejszy — końcówki по группам", headers: ["grupa", "ja", "ty", "on/ona/ono", "my", "wy", "oni/one"], rows: [["-ować / -ywać / -iwać", "-uję", "-ujesz", "-uje", "-ujemy", "-ujecie", "-ują"], ["-ać", "-am", "-asz", "-a", "-amy", "-acie", "-ają"], ["-ić / -yć / część -eć", "-ę", "-isz / -ysz", "-i / -y", "-imy / -ymy", "-icie / -ycie", "-ą"]] },
  ...
  irregularVerbs: [
    { title: "Nieregularne — формы, которые надо знать целиком", headers: ["bezokolicznik", "ja", "ty", "on/ona", "my", "wy", "oni/one"], rows: [["być", "jestem", "jesteś", "jest", "jesteśmy", "jesteście", "są"], ["mieć", "mam", "masz", "ma", "mamy", "macie", "mają"], ["móc", "mogę", "możesz", "może", "możemy", "możecie", "mogą"], ["wiedzieć", "wiem", "wiesz", "wie", "wiemy", "wiecie", "wiedzą"], ["dać", "dam", "dasz", "da", "damy", "dacie", "dadzą"]] },
  ...
  verbsPast: [
    { title: "Czas przeszły — końcówki pełne", headers: ["osoba", "męski", "żeński", "męskoosobowy plural", "niemęskoosobowy plural"], rows: [["ja", "-łem", "-łam", "—", "—"], ["ty", "-łeś", "-łaś", "—", "—"], ["on/ona/ono", "-ł", "-ła / -ło", "—", "—"], ["my", "—", "—", "-liśmy", "-łyśmy"], ["wy", "—", "—", "-liście", "-łyście"], ["oni/one", "—", "—", "-li", "-ły"]] },
  ...
  verbsFuture: [
    { title: "Czas przyszły — końcówki `być`", headers: ["osoba", "forma", "model z bezokolicznikiem", "model z formą przeszłą"], rows: [["ja", "będę", "będę robić", "będę robił / robiła"], ["ty", "będziesz", "będziesz robić", "będziesz robił / robiła"], ["on/ona/ono", "będzie", "będzie robić", "będzie robił / robiła / robiło"], ["my", "będziemy", "będziemy robić", "będziemy robili / robiły"], ["wy", "będziecie", "będziecie robić", "będziecie robili / robiły"], ["oni/one", "będą", "będą robić", "będą robili / robiły"]] },
  ...
  aspect: [
    { title: "Aspekt — пары, которые надо видеть как систему", headers: ["proces / zwyczaj", "rezultat / koniec", "teraźniejszość / przeszłość", "przyszłość"], rows: [["robić", "zrobić", "robię / robiłem", "będę robić / zrobię"], ["pisać", "napisać", "piszę / pisałem", "będę pisać / napiszę"], ["czytać", "przeczytać", "czytam / czytałem", "będę czytać / przeczytam"], ["kupować", "kupić", "kupuję / kupowałem", "będę kupować / kupię"], ["oglądać", "obejrzeć", "oglądam / oglądałem", "będę oglądać / obejrzę"]] },
  ...
  prepositions: [
    { title: "Przyimki + przypadki", headers: ["przyimek", "przypadek", "przykład"], rows: [["do", "dopełniacz", "do sklepu"], ["z = od/skąd", "dopełniacz", "z pracy"], ["z = razem", "narzędnik", "z rodziną"], ["w = gdzie", "miejscownik", "w Polsce"], ["na = gdzie", "miejscownik", "na kursie"], ["na = dokąd", "biernik", "na kurs"], ["o", "miejscownik", "o pracy"]] },
    { title: "Мини-маршруты с предлогами", headers: ["логика", "pytanie", "przykład"], rows: [["движение к месту", "dokąd?", "idę do sklepu / na kurs"], ["нахождение", "gdzie?", "jestem w sklepie / na kursie"], ["движение от места", "skąd?", "wracam ze sklepu / z pracy"], ["вместе с кем", "z kim?", "idę z kolegą"], ["тема разговора", "o czym?", "mówię o egzaminie"], ["после чего", "po czym?", "po pracy odpoczywam"]] }
  ],
  numbersTime: [
  ...
  politeConditional: [
    { title: "Tryb warunkowy i grzeczne prośby", headers: ["forma", "użycie", "przykład"], rows: [["chciałbym/chciałabym", "вежливо хочу", "Chciałbym zapytać o termin."], ["mógłbym/mogłabym", "мог бы / могла бы", "Czy mógłbym zmienić godzinę?"], ["mogliby Państwo", "могли бы Вы", "Czy mogliby Państwo pomóc?"], ["gdybym miał czas", "если бы у меня было время", "Gdybym miał czas, zadzwoniłbym."], ["prosiłbym/prosiłabym", "я бы просил(а)", "Prosiłbym o odpowiedź."]] },
    { title: "Tryb warunkowy — опорные куски", headers: ["funkcja", "model", "przykład"], rows: [["вежливая просьба", "czy mógłbym + bezokolicznik", "Czy mógłbym wejść później?"], ["вежливое желание", "chciałbym + bezokolicznik", "Chciałbym zmienić termin."], ["официальная просьба", "czy mogliby Państwo + bezokolicznik", "Czy mogliby Państwo oddzwonić?"], ["условие", "gdybym..., ...-bym", "Gdybym miał czas, zostałbym."], ["женский род", "-abym", "chciałabym, mogłabym, przyszłabym"]] }
  ],
  imperatives: [
    { title: "Tryb rozkazujący", headers: ["osoba", "forma", "przykład"], rows: [["ty", "zrób / napisz / poczekaj", "Napisz do mnie."], ["wy", "zróbcie / napiszcie", "Poczekajcie chwilę."], ["pan/pani", "proszę + bezokolicznik", "Proszę wypełnić formularz."], ["zakaz", "nie + rozkazujący", "Nie zapomnij dokumentu."], ["instrukcja", "należy / trzeba", "Należy podpisać wniosek."]] },
    { title: "Rozkazujący — полезные модели", headers: ["sytuacja", "model", "przykład"], rows: [["простая просьба", "czasownik rozkazujący", "Powiedz jeszcze raz."], ["обращение к группе", "forma `wy`", "Poczekajcie tutaj."], ["вежливая инструкция", "proszę + bezokolicznik", "Proszę usiąść."], ["запрет", "nie + rozkazujący", "Nie rób tego."], ["официальный запрет", "proszę nie + bezokolicznik", "Proszę nie palić."], ["нейтральная инструкция", "należy / trzeba", "Trzeba podpisać formularz."]] }
  ],
  pronouns: [
    { title: "Zaimki osobowe — pełna ściąga", headers: ["przypadek / pytanie", "ja", "ty", "on", "ona", "my", "wy", "oni/one"], rows: [["mianownik: kto?", "ja", "ty", "on", "ona", "my", "wy", "oni / one"], ["biernik: kogo? co?", "mnie", "cię / ciebie", "go / jego", "ją", "nas", "was", "ich / je"], ["dopełniacz: kogo? czego?", "mnie", "ciebie", "go / niego", "jej", "nas", "was", "ich"], ["celownik: komu? czemu?", "mi / mnie", "ci / tobie", "mu / jemu", "jej", "nam", "wam", "im"], ["narzędnik: z kim? z czym?", "mną", "tobą", "nim", "nią", "nami", "wami", "nimi"], ["miejscownik: o kim? o czym?", "mnie", "tobie", "nim", "niej", "nas", "was", "nich"]] },
    { title: "Zaimki w przypadkach", headers: ["kto?", "biernik", "celownik", "dopełniacz"], rows: [["ja", "mnie", "mi", "mnie"], ["ty", "ciebie / cię", "ci", "ciebie"], ["on", "go / jego", "mu", "go / jego"], ["ona", "ją", "jej", "jej"], ["my", "nas", "nam", "nas"], ["wy", "was", "wam", "was"], ["oni/one", "ich/je", "im", "ich"]] }
  ],
  reflexiveSie: [
    { title: "Konstrukcje z `się` — co po nich idzie", headers: ["czasownik", "model", "przykład", "jaki przypadek dalej?"], rows: [["uczyć się", "uczyć się czegoś", "Uczę się polskiego.", "dopełniacz"], ["bać się", "bać się kogoś/czegoś", "Boję się egzaminu.", "dopełniacz"], ["interesować się", "interesować się czymś", "Interesuję się sportem.", "narzędnik"], ["spotykać się", "spotykać się z kimś", "Spotykam się z kolegą.", "z + narzędnik"], ["podobać się", "podobać się komuś", "Podoba mi się film.", "celownik"], ["czuć się", "czuć się jak?", "Czuję się dobrze.", "opis stanu"]] },
    { title: "Czasowniki z się", headers: ["typ", "przykład", "uwaga"], rows: [["uczyć się", "Uczę się polskiego.", "się обычно после глагола"], ["spotykać się", "Spotykam się z kolegą.", "często z + narzędnik"], ["podobać się", "Podoba mi się kurs.", "кому? = celownik"], ["czuć się", "Czuję się dobrze.", "описание состояния"], ["bać się", "Boję się egzaminu.", "często dopełniacz"]] },
    { title: "Konstrukcje z się — управление", headers: ["czasownik", "pytanie", "model"], rows: [["uczyć się", "czego?", "uczę się języka"], ["bać się", "czego?", "boję się egzaminu"], ["interesować się", "czym?", "interesuję się sportem"], ["spotykać się", "z kim?", "spotykam się z kolegą"], ["podobać się", "komu? co?", "podoba mi się film"], ["czuć się", "jak?", "czuję się dobrze"]] }
  ],
  comparisons: [
    { title: "Stopniowanie — modele", headers: ["stopień", "jak budujemy", "przykład"], rows: [["równy", "forma podstawowa", "dobry, tani, szybki"], ["wyższy", "-szy / -ejszy / forma nieregularna", "tańszy, ciekawszy, lepszy"], ["najwyższy", "naj- + stopień wyższy", "najtańszy, najciekawszy, najlepszy"], ["porównanie dwóch rzeczy", "... niż ...", "Ten kurs jest tańszy niż tamten."], ["równość", "tak samo ... jak ...", "To jest tak samo ważne jak praktyka."]] },
    { title: "Stopniowanie", headers: ["typ", "forma", "przykład"], rows: [["regularne", "-szy / -ejszy", "tani → tańszy"], ["naj-", "najlepszy / najważniejszy", "To najważniejszy temat."], ["nieregularne", "dobry → lepszy → najlepszy", "Ten kurs jest lepszy."], ["porównanie", "niż", "Polski jest trudniejszy niż angielski."], ["równość", "tak samo ... jak", "To jest tak samo ważne jak gramatyka."]] }
  ],
  modalVerbs: [
    { title: "Modalne — końcówki i rodzaj", headers: ["czasownik", "ja", "ty", "on/ona", "my", "wy", "oni/one"], rows: [["musieć", "muszę", "musisz", "musi", "musimy", "musicie", "muszą"], ["móc", "mogę", "możesz", "może", "możemy", "możecie", "mogą"], ["chcieć", "chcę", "chcesz", "chce", "chcemy", "chcecie", "chcą"], ["powinien / powinna", "powinienem / powinnam", "powinieneś / powinnaś", "powinien / powinna", "powinniśmy / powinnyśmy", "powinniście / powinnyście", "powinni / powinny"]] },
    { title: "Czasowniki modalne", headers: ["forma", "znaczenie", "przykład"], rows: [["muszę", "я должен", "Muszę złożyć wniosek."], ["mogę", "я могу", "Mogę przyjść jutro."], ["powinienem/powinnam", "мне следует", "Powinienem powtórzyć słowa."], ["wolno", "разрешено", "Tu wolno parkować."], ["nie wolno", "нельзя", "Nie wolno palić."]] },
    { title: "Modalne — как строится фраза", headers: ["model", "co dalej?", "przykład"], rows: [["muszę", "bezokolicznik", "Muszę zadzwonić."], ["mogę", "bezokolicznik", "Mogę wejść."], ["powinienem", "bezokolicznik", "Powinienem odpocząć."], ["wolno / nie wolno", "bezokolicznik", "Nie wolno palić."], ["rodzaj", "powinienem / powinnam", "Powinnam napisać maila."]] }
  ],
  impersonal: [
    { title: "Formy bezosobowe", headers: ["forma", "użycie", "przykład"], rows: [["można", "можно", "Można zapłacić kartą."], ["trzeba", "нужно", "Trzeba podpisać formularz."], ["należy", "следует", "Należy dołączyć załącznik."], ["mówi się", "говорят", "Mówi się, że to trudne."], ["warto", "стоит", "Warto robić powtórki."]] },
    { title: "Bezosobowe — готовые схемы", headers: ["model", "znaczenie", "przykład"], rows: [["można + bezokolicznik", "можно сделать", "Można wejść."], ["trzeba + bezokolicznik", "нужно сделать", "Trzeba poczekać."], ["należy + bezokolicznik", "следует сделать", "Należy podpisać."], ["warto + bezokolicznik", "стоит сделать", "Warto sprawdzić."], ["mówi się, że...", "говорят, что...", "Mówi się, że to pomaga."]] }
  ],
  wordOrder: [
    { title: "Szyk zdania", headers: ["sytuacja", "schemat", "przykład"], rows: [["neutralnie", "kto + co robi + reszta", "Ja uczę się polskiego."], ["czas na początku", "czas + czasownik + osoba", "Jutro mam egzamin."], ["się", "często po czasowniku", "Uczę się codziennie."], ["nie", "przed czasownikiem", "Nie rozumiem pytania."], ["pytanie", "czy / gdzie / kiedy + reszta", "Kiedy mogę odebrać dokument?"]] },
    { title: "Szyk — главные якоря", headers: ["element", "gdzie zwykle stoi?", "przykład"], rows: [["nie", "przed czasownikiem", "Nie mam czasu."], ["się", "po czasowniku", "Uczę się codziennie."], ["krótkie zaimki", "blisko czasownika", "Podoba mi się film."], ["czas", "часто в начале", "Jutro idę do urzędu."], ["pytanie", "słowo pytające na początku", "Gdzie mieszkasz?"], ["powrót do neutralnego szyku", "kto -> czasownik -> reszta", "Ja pracuję w biurze."]] }
  ],
  b1Connectors: [
    { title: "Łączniki B1", headers: ["łącznik", "znaczenie", "przykład"], rows: [["jednak", "однако", "Chciałem przyjść, jednak nie miałem czasu."], ["natomiast", "а вот / тогда как", "Ja wolę kurs online, natomiast brat woli szkołę."], ["oprócz tego", "кроме того", "Uczę się słów, oprócz tego czytam teksty."], ["z tego powodu", "по этой причине", "Był remont, z tego powodu autobus się spóźnił."], ["podsumowując", "подводя итог", "Podsumowując, to dobry pomysł."]] },
    { title: "Связки по функции", headers: ["funkcja", "łączniki", "mini-model"], rows: [["причина", "bo, ponieważ", "Nie przyszedłem, bo byłem chory."], ["результат", "dlatego, z tego powodu", "Byłem chory, dlatego zostałem w domu."], ["противопоставление", "jednak, natomiast", "Lubię teorię, jednak wolę praktykę."], ["добавление", "oprócz tego", "Czytam teksty, oprócz tego słucham podcastów."], ["вывод", "podsumowując", "Podsumowując, to dobry plan."]] }
  ],
  b1Mistakes: [
    { title: "Типы ошибок B1", headers: ["typ błędu", "na co patrzeć?", "przykład"], rows: [["падеж", "какой вопрос?", "pomagam koledze"], ["вид/аспект", "процесс или результат?", "będę pisać / napiszę"], ["порядок слов", "где `nie`, `się`, местоимение?", "Podoba mi się film."], ["предлог", "какой смысл у предлога?", "na kurs / na kursie"], ["местоимение", "komu? kogo? czego?", "daj mi / widzę go"], ["связка", "причина, результат или вывод?", "bo / dlatego / podsumowując"]] }
  ]
};

const topicRuleSheets = {
  nominative: [
    { title: "Что такое mianownik", body: "Mianownik — это базовая словарная форма. Он отвечает на pytanie `kto? co?` и нужен, когда мы просто называем человека, вещь, профессию или предмет разговора: `to jest lekarz`, `to jest kawa`, `to jest dziecko`." },
    { title: "Как мыслить в этой теме", body: "Здесь не нужно сначала что-то ломать. Наоборот: сначала привыкаем к форме без изменений. Полезный алгоритм такой: 1. кто или что? 2. какой это род? 3. как согласовать прилагательное? Поэтому: `dobry lekarz`, `dobra kawa`, `małe dziecko`." },
    { title: "Где чаще всего ошибка", body: "Самая частая ошибка в mianownik — не сам noun, а согласование: `dobra lekarz`, `mały kawa`, `nowa mieszkanie`. Учить нужно не одно слово, а пару целиком: `dobry student`, `miła kobieta`, `nowe biuro`." }
  ],
  pluralNominative: [
    ...
    { title: "Какие окончания здесь встречаются", body: "В męskoosobowy часто видим формы типа `-i / -y / -e / -owie`: `student -> studenci`, `lekarz -> lekarze`, `pan -> panowie`, `kolega -> koledzy`.\n\nВ niemęskoosobowy часто встречаются `-y / -i / -e / -a`: `kobieta -> kobiety`, `pies -> psy`, `dziecko -> dzieci`, `okno -> okna`." },
    { title: "Где чаще всего ошибка", body: "Обычно ошибка не в самом слове, а в согласовании: человек ставит `dobre studenci` или `polskie lekarze`. Здесь надо видеть пару целиком: прилагательное + существительное. То есть не слово само по себе, а блок: `dobrzy studenci`, `mili koledzy`, `dobre dzieci`, `nowe mieszkania`." }
  ],
  accusativePlural: [
    { title: "Что это за отдельная тема", body: "Biernik liczby mnogiej нельзя прятать внутрь общей темы как короткое примечание. Здесь есть две разные линии, и именно из-за них люди спотыкаются.\n\n1. `męskoosobowy`: `widzę dobrych studentów`, `znam nowych pracowników`\n2. `niemęskoosobowy`: `widzę dobre książki`, `kupuję nowe bilety`, `mam małe dzieci`\n\nСначала решай, о какой группе речь, и только потом меняй форму." },
    { title: "Первая линия: męskoosobowy", body: "Если речь о мужчинах, группах мужчин или смешанной группе людей, очень часто получаем модель:\n- существительное как в `dopełniacz liczby mnogiej`\n- прилагательное: `-i / -y` -> `-ych / -ich`\n\nПримеры:\n`dobrzy studenci -> widzę dobrych studentów`\n`nowi pracownicy -> spotykam nowych pracowników`\n`mili koledzy -> znam miłych kolegów`\n\nПолезный быстрый ориентир: если это люди мужского типа, очень часто увидишь `-ów / -y / -i` в конце существительного и `-ych / -ich` у прилагательного." },
    { title: "Вторая линия: niemęskoosobowy", body: "Если это женщины, вещи, документы, города, блюда, дети, животные или просто немужская группа, biernik liczby mnogiej очень часто совпадает с mianownik.\n\nПримеры:\n`dobre książki -> widzę dobre książki`\n`nowe dokumenty -> mam nowe dokumenty`\n`małe dzieci -> widzę małe dzieci`\n`ciekawe filmy -> oglądam ciekawe filmy`\n\nТо есть здесь часто не надо ничего дополнительно «ломать»: форма уже готова в mianownik liczby mnogiej." },
    { title: "Как меняются прилагательные", body: "Это одна из самых важных опор темы.\n\n`męskoosobowy`:\n`dobrzy -> dobrych`\n`nowi -> nowych`\n`polscy -> polskich`\n`mili -> miłych`\n\n`niemęskoosobowy`:\n`dobre -> dobre`\n`nowe -> nowe`\n`ciekawe -> ciekawe`\n`małe -> małe`\n\nПоэтому нельзя учить только существительное. Нужно видеть целую пару: `dobrych studentów`, но `dobre książki`." },
    { title: "Где тут ломается автоматизм", body: "Самые частые ошибки такие:\n- делать `męskoosobowy` как mianownik: `widzę dobrzy studenci`\n- делать `niemęskoosobowy` как dopełniacz: `mam nowych dokumentów`\n- забывать прилагательное: `widzę dobry studentów`\n\nРабочий алгоритм:\n1. кто или что?\n2. `męskoosobowy` или `niemęskoosobowy`?\n3. только после этого выбираем форму существительного и прилагательного." }
  ],
  accusative: [
  ...
  genitivePlural: [
    { title: "Почему to osobna тема", body: "Dopełniacz liczby mnogiej — это не маленькое приложение к единственному числу. Здесь свои окончания, свои ловушки и очень частые конструкции: `nie ma wolnych miejsc`, `potrzebuję nowych dokumentów`, `szukam tanich biletów`." },
    { title: "Три самые частые модели окончаний", body: "Во множественном числе нужно узнавать хотя бы три опоры:\n\n1. `-ów`\n`studentów`, `dokumentów`, `pracowników`, `biletów`\n\n2. `-i / -y`\n`książek` нет, но `aplikacji`, `wiadomości`, `noży`, `ludzi`\n\n3. zero ending / форма без явного окончания\n`kobiet`, `spraw`, `miejsc`, `mieszkań`\n\nЭто не хаос ради хаоса. Это просто надо увидеть как отдельную систему и набить сериями." },
    { title: "Когда эта тема нужна в речи", body: "Самые частые триггеры такие:\n- отрицание: `nie ma wolnych pokoi`\n- количество: `dużo ludzi`, `mało błędów`, `trochę pieniędzy`\n- глаголы: `szukam`, `potrzebuję`, `używam`, `słucham`\n\nТо есть это не редкая книжная тема, а очень живая часть повседневного польского." },
    { title: "Как вести себя прилагательному", body: "Прилагательное в dopełniacz liczby mnogiej очень часто даёт формы `-ych / -ich`:\n`wolne miejsca -> wolnych miejsc`\n`nowe dokumenty -> nowych dokumentów`\n`ciekawe podcasty -> ciekawych podcastów`\n`dobre książki -> dobrych książek`\n\nДаже если существительное ведёт себя по-разному, прилагательное здесь часто даёт хорошую подсказку." },
    { title: "Что надо учить как готовые куски", body: "Самый полезный путь — запоминать не сухие слова, а связки:\n`nie ma wolnych miejsc`\n`szukam tanich mieszkań`\n`potrzebuję nowych dokumentów`\n`używam kolorowych długopisów`\n`dużo ważnych informacji`\n\nИменно так тема начинает жить, а не распадаться на случайные окончания." }
  ],
  genitive: [
    ...
    { title: "На какие окончания опираться", body: "В единственном числе полезно видеть такие ориентиры:\n- мужской род часто `-a / -u`: `studenta`, `telefonu`, `czasu`\n- женский род часто `-y / -i`: `kawy`, `pracy`, `pomocy`\n- средний род часто `-a`: `dziecka`, `mieszkania`\n\nВо множественном числе часто встречаются формы `-ów`, `-i / -y` или форма без окончания: `studentów`, `ludzi`, `kobiet`, `książek`." },
    { title: "Главная ловушка", body: "Не пытайся вывести все окончания одной формулой. Здесь лучше помнить частые блоки: `nie mam czasu`, `szukam pracy`, `potrzebuję pomocy`, `dużo ludzi`. Особенно внимательно смотри на конструкции после `dużo / mało / trochę` и после отрицания `nie ma / nie mam`." }
  ],
  dativePlural: [
    { title: "Что здесь главное", body: "Celownik liczby mnogiej — это тема про адресата во множественном числе: `komu?` не одному человеку, а группе.\n\n`pomagam rodzicom`\n`daję zadanie studentom`\n`wysyłam wiadomość klientom`\n`tłumaczę zasady uczestnikom`" },
    { title: "Базовое окончание, которое надо увидеть", body: "Главная опора здесь — окончание `-om`.\n\n`student -> studentom`\n`klient -> klientom`\n`rodzice -> rodzicom`\n`dzieci -> dzieciom`\n`kobiety -> kobietom`\n`grupy -> grupom`\n\nЕсли задача про `komu?` и речь о множественном числе, очень часто именно `-om` вытащит тебя к правильной форме." },
    { title: "Как ведёт себя прилагательное", body: "Прилагательное в celownik liczby mnogiej обычно даёт форму `-ym / -im`:\n`nowi studenci -> nowym studentom`\n`mili klienci -> miłym klientom`\n`małe dzieci -> małym dzieciom`\n`nowe grupy -> nowym grupom`\n\nПолезно видеть всю пару целиком, а не только конец существительного." },
    { title: "Какие глаголы чаще всего требуют эту форму", body: "Чаще всего celownik liczby mnogiej приходит после таких глаголов:\n`pomagać`\n`dawać`\n`mówić`\n`tłumaczyć`\n`pokazywać`\n`wysyłać`\n`odpowiadać`\n\nУчить лучше сразу как блок: `pomagam rodzicom`, `wysyłam link znajomym`, `mówię prawdę kolegom`." },
    { title: "Типичные ошибки", body: "Люди часто ставят здесь biernik или dopełniacz:\n`pomagam nowych studentów` — ошибка\n`daję książki dzieci` — ошибка\n\nПравильный вопрос здесь всегда один: `komu?`.\nЕсли это получатель действия и их много, очень часто нужно `-om`." }
  ],
  dative: [
    ...
    { title: "На какие окончания смотреть", body: "Полезные ориентиры такие:\n- мужской род часто `-owi / -u`: `studentowi`, `lekarzowi`, `bratu`\n- женский род часто `-ie / -y`: `kobiecie`, `mamie`, `siostrze`\n- средний род часто `-u`: `dziecku`\n- во множественном числе очень часты формы `-om`: `studentom`, `kobietom`, `dzieciom`, `ludziom`" },
    { title: "Главная ловушка", body: "После `pomagać`, `dawać`, `mówić`, `pokazywać` нельзя машинально ставить biernik. Сначала спроси: это объект действия или адресат? В `daję książkę bratu` книга — это объект, а брат — адресат. Поэтому здесь одновременно есть biernik и celownik." }
  ],
  instrumentalPlural: [
    { title: "Зачем выносить это отдельно", body: "Narzędnik liczby mnogiej — это не просто строка про `-ami`. Здесь много очень частых живых конструкций: `z ludźmi`, `z klientami`, `językami`, `projektami`, `dziećmi`, `rodzicami`." },
    { title: "Две основные формы, которые надо знать", body: "Во множественном числе чаще всего встречаются две опоры:\n\n1. `-ami`\n`klientami`, `projektami`, `językami`, `grupami`\n\n2. `-mi`\n`dziećmi`, `ludźmi`\n\nИменно поэтому тему надо учить отдельными сериями: здесь много очень частых слов с нетривиальной формой." },
    { title: "Где эта тема живёт", body: "Самые частые конструкции такие:\n`z kim? z czym?`\n`interesować się czym?`\n`zajmować się czym?`\n`między kim? czym?`\n`pracować z kim?`\n\nТо есть narzędnik liczby mnogiej — это не редкость, а нормальная повседневная речь о людях, вещах и занятиях." },
    { title: "Как ведёт себя прилагательное", body: "Прилагательное в narzędnik liczby mnogiej обычно даёт форму `-ymi / -imi`:\n`nowi klienci -> nowymi klientami`\n`obce języki -> obcymi językami`\n`ważne projekty -> ważnymi projektami`\n`małe dzieci -> małymi dziećmi`\n\nКак только видишь `z`, `interesuję się`, `zajmuję się`, сразу проверяй этот тип согласования." },
    { title: "Что надо добить до автоматизма", body: "Запоминай готовые куски:\n`pracuję z ludźmi`\n`rozmawiam z klientami`\n`interesuję się językami`\n`zajmuję się projektami`\n`idę z dziećmi`\n\nТак форма закрепляется как часть мысли, а не как голое окончание." }
  ],
  instrumental: [
    ...
    { title: "Типовые окончания", body: "Полезно помнить такие ориентиры:\n- мужской и средний род часто `-em`: `studentem`, `samochodem`, `dzieckiem`\n- женский род часто `-ą`: `kobietą`, `rodziną`, `książką`\n- множественное число часто `-ami / -mi`: `studentami`, `samochodami`, `ludźmi`" },
    { title: "Практический способ", body: "Лучше учить готовыми кусками: `jadę samochodem`, `rozmawiam z nauczycielem`, `jestem studentem`, `pracuję z ludźmi`. Тогда narzędnik становится не таблицей, а привычной частью живой фразы." }
  ],
  locativePlural: [
    { title: "Что это за линия", body: "Miejscownik liczby mnogiej нужно видеть как отдельный набор форм для `o`, `w`, `na`, `po`, `przy` во множественном числе.\n\nПримеры:\n`o planach`\n`w sklepach`\n`na zajęciach`\n`o książkach`\n`w miastach`" },
    { title: "Главная модель", body: "Самый частый ориентир — окончание `-ach`.\n\n`plany -> planach`\n`sklepy -> sklepach`\n`zajęcia -> zajęciach`\n`książki -> książkach`\n`miasta -> miastach`\n\nЕсли задача про `где?` или `о чём?` и речь о множественном числе, очень часто всплывёт именно `-ach`." },
    { title: "Как ведёт себя прилагательное", body: "Прилагательное в miejscownik liczby mnogiej обычно даёт форму `-ych / -ich`:\n`ważne plany -> o ważnych planach`\n`duże sklepy -> w dużych sklepach`\n`ciekawe zajęcia -> na ciekawych zajęciach`\n`polskie miasta -> o polskich miastach`\n\nПоэтому полезно видеть сразу целый блок: прилагательное + существительное + предлог." },
    { title: "Какие предлоги чаще всего приводят сюда", body: "Чаще всего это:\n`o` — о чём?\n`w` — где?\n`na` — где?\n`po` — после / по чему?\n`przy` — при / рядом с чем?\n\nНапример:\n`rozmawiam o problemach`\n`jestem w sklepach`\n`uczę się na zajęciach`\n`myślę o planach`" },
    { title: "Типичные ошибки", body: "Самые частые сбои такие:\n- оставить mianownik: `o ważne plany`\n- спутать с narzędnik: `o ważnymi planami`\n- забыть, что предлог требует форму всей группы, а не только существительного\n\nРабочая модель: `предлог -> вопрос -> liczba mnoga -> -ach / -ych / -ich`." }
  ],
  locative: [
    ...
    { title: "Какие формы часто встречаются", body: "Очень часто здесь видим формы типа `-e / -ie / -u`:\n`Polsce`, `kursie`, `sklepie`, `języku`, `parku`, `biurze`, `spotkaniu`.\n\nЭто не значит, что всегда будет одно и то же окончание, но такие формы стоит узнавать сразу." },
    { title: "Главная ловушка", body: "Предлог `na` может тянуть разные падежи: `na kurs` = куда? biernik, а `na kursie` = где? miejscownik. Точно так же `w szkołę` не бывает, потому что после `w` при значении `где` нужен miejscownik: `w szkole`." }
  ],
  verbsPresent: [
    { title: "Главная идея темы", body: "Настоящее время в польском нельзя учить как список переводов. Каждый глагол нужно сразу видеть как модель по лицам: `ja`, `ty`, `on/ona`, `my`, `wy`, `oni/one`." },
    { title: "Три основные модели, которые надо узнать", body: "На старте тебе нужны три главные линии:\n\n1. `-ować / -ywać / -iwać` -> `-uję, -ujesz, -uje, -ujemy, -ujecie, -ują`\n`pracować -> pracuję, pracujesz`\n`pokazywać -> pokazuję, pokazujesz`\n\n2. часть глаголов на `-ać` -> `-am, -asz, -a, -amy, -acie, -ają`\n`mieszkać -> mieszkam, mieszkasz`\n`czytać -> czytam, czytasz`\n\n3. многие глаголы на `-ić / -yć / -eć` -> `-ę, -isz / -ysz, -i / -y, -imy / -ymy, -icie / -ycie, -ą`\n`mówić -> mówię, mówisz`\n`uczyć -> uczę, uczysz`\n`widzieć -> widzę, widzisz`" },
    { title: "Как распознать модель", body: "Сначала смотри на bezokolicznik. Если видишь `-ować`, почти всегда ожидай `pracuję / pracujesz`. Если видишь частый глагол на `-ać`, часто проверяй модель `-am / -asz`. Если видишь `-ić / -yć / -eć`, очень часто появится `-ę / -isz / -ysz`.\n\nНо главное правило такое: модель угадываем по типу глагола, а потом обязательно проверяем по частым формам `ja` и `ty`." },
    { title: "Что особенно важно для osoby `ty`", body: "Именно форма `ty` лучше всего показывает группу:\n- `pracujesz` -> модель `-ujesz`\n- `mieszkasz` -> модель `-asz`\n- `mówisz`, `uczysz`, `widzisz` -> модель `-isz / -ysz`\n\nПоэтому, если форма `ty` понятна, половина спряжения уже у тебя в руках." },
    { title: "Что надо учить отдельно", body: "Есть глаголы, где меняется не только окончание, но и основа. Их надо узнавать сразу как готовые слова: `być`, `mieć`, `iść`, `jeść`, `móc`, `brać`, `dać`, `wiedzieć`, `chcieć`." },
    { title: "Как учить по-настоящему полезно", body: "Не `pracować = работать`, а `pracuję w firmie`. Не `mówić = говорить`, а `mówię po polsku`. Не `mieć = иметь`, а `mam czas`, `nie mam czasu`.\n\nТо есть учим не отдельную форму, а сразу короткую живую реплику." }
  ],
  irregularVerbs: [
    { title: "Как к ним относиться", body: "Неправильные глаголы не надо пытаться вывести логикой до конца. Их надо узнавать как частые готовые формы, потому что они живут почти в каждом разговоре." },
    { title: "Какие формы учить первыми", body: "Сначала бери три опорные точки:\n- `ja`\n- `ty`\n- `oni/one`\n\nИменно они чаще всего ломают автоматизм:\n`idę -> idziesz -> idą`\n`biorę -> bierzesz -> biorą`\n`dam -> dasz -> dadzą`\n`wiem -> wiesz -> wiedzą`" },
    { title: "Чего не делать", body: "Не надо превращать этот блок в бесконечную таблицу без жизни. Сначала узнавай самые частые формы, потом закрепляй их в готовой фразе: `idę do pracy`, `mogę wejść`, `nie wiem`, `dam ci znać`, `oni jedzą obiad`." },
    { title: "Минимальный набор, без которого тяжело", body: "Если человек учит польский для жизни, он должен очень уверенно узнавать:\n`być`, `mieć`, `iść`, `jechać`, `jeść`, `móc`, `chcieć`, `wiedzieć`, `brać`, `dać`.\n\nЭто не “дополнительные слова”, а каркас речи." }
  ],
  verbsPast: [
    { title: "Что здесь главное", body: "В прошедшем времени сначала смотри не на окончание инфинитива, а на того, кто говорит. Здесь род и число влияют на форму так сильно, что без этого шагa нельзя собрать правильный глагол." },
    { title: "Как собирать форму", body: "Самая полезная опора такая:\n- `ja` мужчина -> часто `-łem`: `robiłem`, `mieszkałem`\n- `ja` женщина -> часто `-łam`: `robiłam`, `mieszkałam`\n- `ty` мужчина -> `-łeś`\n- `ty` женщина -> `-łaś`\n- `my` смешанная / мужская группа -> `-liśmy`\n- `my` только женщины / niemęskoosobowy -> `-łyśmy`" },
    { title: "Что надо помнить отдельно", body: "Глаголы движения и часть частых глаголов надо запоминать как блоки:\n`poszedłem / poszłam`\n`przyszedłem / przyszłam`\n`pojechałem / pojechałam`\n\nИменно они чаще всего встречаются в рассказе о вчерашнем дне, выходных и планах, которые уже состоялись." },
    { title: "Где больше всего ошибок", body: "Обычно ошибка в одном из трёх мест:\n1. забыли род\n2. перепутали `-li-` и `-ły-`\n3. взяли неправильную основу у глагола движения\n\nПоэтому алгоритм всегда такой: кто говорит -> какой род / число -> только потом форма." }
  ],
  verbsFuture: [
    { title: "Сначала смысл, потом форма", body: "В будущем времени сначала решай не окончание, а смысл: ты говоришь о процессе, который будет длиться, или о результате, который случится." },
    { title: "Две линии будущего", body: "1. Несовершенный вид -> процесс\n`będę robić`, `będę czytać`, `będę pracować`\nИногда ещё: `będę robił / będę robiła`\n\n2. Совершенный вид -> результат\n`zrobię`, `napiszę`, `przeczytam`, `kupię`\n\nТо есть будущее в польском строится через выбор вида, а не только через одно окончание." },
    { title: "Как это понять по-русски", body: "Если по смыслу ближе `буду делать / буду читать / буду работать`, чаще нужна конструкция с `będę`. Если ближе `сделаю / прочитаю / куплю`, чаще нужен совершенный глагол без `będę`." },
    { title: "Главная ловушка", body: "Нельзя смешивать две модели: `będę zrobię` не бывает. Надо выбрать либо процесс `będę robić`, либо результат `zrobię`." }
  ],
  aspect: [
    { title: "Что такое aspekt по-человечески", body: "Аспект в польском — это выбор между процессом и результатом. Ты либо показываешь ход действия, повторяемость, длительность, либо подчёркиваешь, что действие завершится или уже завершилось." },
    { title: "Как выбирать", body: "К процессу часто тянут слова `często`, `zwykle`, `codziennie`, `teraz`, `długo`. К результату часто тянут слова `już`, `w końcu`, `na jutro`, `do końca`, `raz` в значении одного завершённого действия." },
    { title: "Как это учить", body: "Не отдельно `robić` и отдельно `zrobić`, а именно парами:\n`czytać / przeczytać`\n`pisać / napisać`\n`robić / zrobić`\n`kupować / kupić`\n`oglądać / obejrzeć`\n\nИ сразу с вопросом: где здесь процесс, а где результат?" },
    { title: "Главная ошибка", body: "Ошибка не только в форме, а в смысле. Если человек хочет сказать `я завтра закончу письмо`, а выбирает процесс вместо результата, то мысль уже звучит иначе. Поэтому аспект — это смысловой выбор, а не украшение таблицы." }
  ],
  prepositions: [
    ...
    { title: "Мини-маршруты, которые надо знать", body: "Очень полезны такие цепочки:\n`idę do sklepu -> jestem w sklepie -> wracam ze sklepu`\n`idę na kurs -> jestem na kursie -> wracam z kursu`\n`jadę do pracy -> jestem w pracy -> wracam z pracy`\n\nИменно так предлоги перестают быть набором слов и превращаются в систему." },
    { title: "Где чаще всего путаются", body: "В парах `do / w / na`, а ещё в предлоге `z`, потому что он может значить и `с кем`, и `откуда`. Ещё одна частая ошибка — смотреть только на перевод, а не на вопрос: нужно всегда сначала понять `куда?`, `где?` или `откуда?`." }
  ],
  numbersTime: [
    ...
    { title: "Три главные числовые модели", body: "Смотри на последние цифры:\n- `1` -> обычно форма единственного числа: `jeden złoty`, `jedna godzina`\n- `2, 3, 4` -> форма множественного числа: `dwa złote`, `trzy godziny`\n- `5+` и числа типа `12, 13, 14` -> dopełniacz liczby mnogiej: `pięć złotych`, `piętnaście minut`, `dwanaście godzin`" },
    { title: "Как учить полезно", body: "Не отдельно числа и отдельно часы, а через реальные блоки: `Spotkanie jest o ósmej`, `Bilet kosztuje dwadzieścia dwa złote`, `Za godzinę wychodzę`. Тогда тема сразу становится жизненной, а не абстрактной." }
  ],
  complexSentences: [
    { title: "Почему это уже B1", body: "На B1 мало назвать факт. Нужно уметь добавить причину, мнение, цель, условие, пример или вывод. Именно поэтому простые предложения начинают соединяться в более длинную мысль." },
    { title: "Как строить длинную мысль", body: "Самый полезный каркас такой:\n1. факт или мнение\n2. причина / пояснение\n3. результат / вывод\n\nНапример:\n`Myślę, że to dobry pomysł, bo jest wygodny. Dlatego chcę spróbować.`\n\nЭто уже нормальный B1-ответ." },
    { title: "Какие связки нужны в первую очередь", body: "Для самостоятельной речи достаточно уверенно владеть такими опорами:\n`że` — что\n`bo / ponieważ` — потому что\n`dlatego` — поэтому\n`żeby` — чтобы\n`jeśli / jeżeli` — если\n`kiedy / gdy` — когда" },
    { title: "Главная ловушка", body: "Не нужно строить одно бесконечное предложение. Сильная речь — это 2–3 простые части, соединённые правильно. Сначала мысль, потом связка, потом следующая часть." }
  ],
  politeConditional: [
    { title: "Зачем нужен tryb warunkowy", body: "Этот блок нужен не для абстрактной грамматики, а для вежливой и взрослой речи. Именно здесь появляются формы, которые делают просьбу мягкой: `chciałbym`, `mógłbym`, `czy mogliby Państwo...`." },
    { title: "Как строить вежливую просьбу", body: "Самая полезная модель такая:\n`chciałbym / chciałabym + bezokolicznik`\n`mógłbym / mogłabym + bezokolicznik`\n`czy mogliby Państwo + bezokolicznik`\n\nНапример:\n`Chciałbym zapytać o termin.`\n`Czy mógłbym zmienić godzinę?`\n`Czy mogliby Państwo wysłać odpowiedź mailem?`" },
    { title: "Условие и результат", body: "Если говорим о гипотетической ситуации, часто используем пару:\n`gdybym ... , ... -bym`\n\nНапример:\n`Gdybym miał czas, zadzwoniłbym.`\n`Gdybym wiedziała wcześniej, przyszłabym.`\n\nТо есть первая часть задаёт условие, а вторая показывает, что бы произошло." },
    { title: "Где ошибаются", body: "Частая ошибка — путать условное с обычным будущим или просто ставить инфинитив без формы `-bym`. Здесь надо помнить: если речь о вежливой просьбе или гипотетической ситуации, чаще всего нужна именно условная форма." }
  ],
  imperatives: [
    { title: "Что такое tryb rozkazujący", body: "Это не только `приказ`. В польском он нужен для инструкций, просьб, советов, напоминаний и запретов." },
    { title: "Три главные модели", body: "1. Для `ty`: `zrób`, `napisz`, `poczekaj`\n2. Для `wy`: `zróbcie`, `napiszcie`, `poczekajcie`\n3. Для вежливой формы `pan/pani/państwo` очень часто используется:\n`proszę + bezokolicznik`\nНапример: `Proszę podpisać formularz.`" },
    { title: "Как строить запрет", body: "Запрет часто строится так:\n`nie + forma rozkazująca`\nили более официально:\n`proszę nie + bezokolicznik`\n\nНапример:\n`Nie zapomnij dokumentu.`\n`Proszę nie palić.`" },
    { title: "Как учить полезно", body: "Лучше учить не отдельные формы, а готовые жизненные блоки:\n`napisz do mnie`\n`powiedz jeszcze raz`\n`proszę poczekać`\n`proszę wypełnić formularz`\n`nie zapomnij o spotkaniu`" }
  ],
  pronouns: [
    ...
    { title: "Самые полезные пары", body: "Полезно держать в голове такие блоки:\n`pomóż mi`\n`powiedz mi`\n`widzę cię`\n`znam go`\n`daj jej znać`\n`nie ma nas`\n`pomagam im`\n\nЕсли местоимение живёт рядом с глаголом, форма запоминается намного легче." },
    { title: "Как закреплять", body: "Только в парах с глаголом: `pomóż mi`, `powiedz jej`, `znam go`, `potrzebuję ich`. То есть не `mi = мне`, а `daj mi`, не `go = его`, а `widzę go`." }
  ],
  reflexiveSie: [
    { title: "Что делать с się", body: "Нельзя учить `się` как отдельное слово. Его надо учить только вместе с глаголом и всей моделью: `uczyć się czegoś`, `bać się czegoś`, `spotykać się z kimś`, `podobać się komuś`." },
    { title: "Как мыслить", body: "Если глагол живёт с `się`, он часто меняет не только форму, но и управление. Поэтому здесь нужно сразу видеть весь блок: какой глагол, какой падеж после него, кому или чего касается действие." },
    { title: "Что особенно важно", body: "Есть минимум четыре очень частые модели:\n`uczyć się czegoś`\n`bać się czegoś`\n`spotykać się z kimś`\n`podobać się komuś`\n\nЕсли их знать как готовые формулы, тема перестаёт быть хаотичной." },
    { title: "Главная ловушка", body: "Люди часто теряют `się` или ставят его не туда. Поэтому здесь нельзя учить перевод отдельно. Нужно довести до привычки полную фразу: `uczę się polskiego`, `boję się psa`, `spotykam się z kolegą`, `podoba mi się film`." }
  ],
  comparisons: [
    ...
    { title: "Три полезные модели", body: "1. Сравнение двух вещей: `A jest lepszy niż B`\n2. Самая высокая степень: `to jest najlepszy вариант`\n3. Равенство: `to jest tak samo ważne jak tamto`\n\nЭти три модели уже закрывают большую часть B1-ситуаций." },
    { title: "Как использовать", body: "Лучше не отдельно слово, а мысль целиком: `Ten вариант jest wygodniejszy niż tamten`, `To było najlepsze rozwiązanie`, `To nie jest tak samo ważne jak zdrowie`." }
  ],
  modalVerbs: [
    { title: "Почему это практичная тема", body: "Модальные конструкции нужны каждый день: попросить разрешение, сказать о необходимости, дать совет, отказаться или объяснить правило." },
    { title: "Базовая модель", body: "Самая важная формула такая:\n`modalny + bezokolicznik`\n\n`muszę iść`\n`mogę wejść`\n`powinienem zadzwonić`\n`trzeba poczekać`\n\nЕсли это понять, половина бытовых ситуаций уже закрывается." },
    { title: "Как различать смысл", body: "`musieć` — обязанность\n`móc` — возможность / разрешение\n`powinien / powinna` — совет, рекомендация\n`trzeba` — безличная необходимость\n\nЭто полезно учить не как перевод, а как разные жизненные роли." },
    { title: "Где ошибка", body: "Самая частая ошибка — после модального ставить уже спрягаемую форму: не `mogę idę`, а `mogę iść`. Здесь всегда нужно проверять: после модального должен идти bezokolicznik." }
  ],
  impersonal: [
    { title: "Зачем нужны безличные формы", body: "Они постоянно встречаются в объявлениях, правилах, инструкциях, письмах и официальных сообщениях. Это не редкая грамматика, а нормальный язык города и учреждений." },
    { title: "Как их понимать", body: "Это способ говорить нейтрально, без конкретного `я` или `ты`: `można`, `trzeba`, `należy`, `warto`. Смысл такой: действие важно само по себе, а не то, кто именно его делает." },
    { title: "Как строить фразу", body: "Самые полезные схемы такие:\n`trzeba + bezokolicznik`\n`można + bezokolicznik`\n`warto + bezokolicznik`\n`należy + bezokolicznik`\n\nНапример:\n`Trzeba podpisać formularz.`\n`Można zapłacić kartą.`" },
    { title: "Где это особенно пригодится", body: "Этот блок нужен для речи типа:\n`tu nie można palić`\n`należy wypełnić wniosek`\n`warto wcześniej zadzwonić`\n\nТо есть для правил, советов и нейтральных инструкций." }
  ],
  wordOrder: [
    { title: "Почему порядок слов пугает", body: "Потому что в польском порядок слов гибкий, но не хаотичный. Если потерялся, всегда возвращайся к нейтральной модели, и фраза уже будет звучать нормально." },
    { title: "Базовая опора", body: "Нейтральная схема такая:\n`кто / тема -> глагол -> остальная информация`\n\n`Ja pracuję w biurze.`\n`My mieszkamy w Warszawie.`\n`Ona jutro dzwoni do urzędu.`" },
    { title: "Главные якоря", body: "`nie` обычно стоит перед глаголом: `nie mam czasu`\n`się` часто идёт после глагола или рядом с ним: `uczę się codziennie`\nкороткие местоимения держатся близко к глаголу: `podoba mi się film`, `daj mi znać`" },
    { title: "Как не ломать фразу", body: "Если предложение становится тяжёлым, не пытайся украшать его перестановками. Сначала собери нейтральную версию, а уже потом переноси время или уточнение в начало: `Jutro mam spotkanie`, `Po pracy idę do sklepu`." }
  ],
  b1Connectors: [
    { title: "Почему связки важны", body: "Связки делают из отдельных предложений нормальную взрослую речь уровня B1. Без них ответ звучит как набор коротких фраз без логики." },
    { title: "Как использовать", body: "Не нужно знать двадцать связок. Достаточно уверенно владеть несколькими по функциям:\n- причина\n- результат\n- противопоставление\n- добавление\n- вывод" },
    { title: "Практический каркас", body: "Очень рабочая схема для B1 такая:\n`Moim zdaniem...`\n`ponieważ / bo...`\n`jednak...`\n`oprócz tego...`\n`podsumowując...`\n\nДаже с этим набором уже можно сделать связный ответ, письмо или мини-мнение." },
    { title: "Где ошибаются", body: "Частая ошибка — использовать связку как красивое слово без логики. Сначала реши, что именно ты добавляешь: причину, результат, контраст или вывод. Только потом выбирай `bo`, `dlatego`, `jednak`, `oprócz tego`, `podsumowując`." }
  ],
  b1Mistakes: [
    { title: "Что это за блок", body: "Это не просто список ошибок. Это место, где надо учиться видеть тип проблемы: падеж, время, аспект, порядок слов, `się`, местоимение или связка." },
    { title: "Как работать с ошибкой правильно", body: "Не просто исправь форму, а всегда объясни себе причину:\n- какой здесь вопрос?\n- какой нужен падеж?\n- это процесс или результат?\n- где должен стоять глагол, `nie`, `się`?\n\nТолько так ошибка перестаёт повторяться." },
    { title: "Главный принцип B1", body: "Если одна и та же ошибка повторяется несколько раз, это не “случайность”, а тема, которую надо доучить как правило. Поэтому этот блок нужен не в конце ради галочки, а как возврат к слабому месту." }
  ]
};
...
function genNominativeAgreement() {
  const makeChoice = (q, options, correct, explanation = "") => choice(q, shuffle(options), correct, explanation);
  const items = [
    makeChoice("Wybierz poprawną formę: To jest ___", ["dobry lekarz", "dobra lekarz", "dobre lekarz"], "dobry lekarz"),
    makeChoice("Wybierz poprawną formę: To jest ___", ["miła koleżanka", "miły koleżanka", "miłe koleżanka"], "miła koleżanka"),
    makeChoice("Wybierz poprawną formę: To jest ___", ["małe dziecko", "mały dziecko", "mała dziecko"], "małe dziecko"),
    makeChoice("Wybierz poprawną formę: To jest ___", ["nowy dokument", "nowa dokument", "nowe dokument"], "nowy dokument"),
...
function genNominativeIdentity() {
  return cap50([
    input("Uzupełnij przymiotnik w mianowniku: To jest ___ lekarz (dobry).", "dobry"),
    input("Uzupełnij przymiotnik w mianowniku: To jest ___ kobieta (miły).", "miła"),
    input("Uzupełnij przymiotnik w mianowniku: To jest ___ dziecko (mały).", "małe"),
    input("Uzupełnij przymiotnik w mianowniku: To jest ___ restauracja (polski).", "polska"),
    input("Uzupełnij przymiotnik w mianowniku: To jest ___ mieszkanie (nowy).", "nowe"),
    input("Uzupełnij przymiotnik w mianowniku: To jest ___ problem (ważny).", "ważny"),
    choice("Wybierz poprawnie: To jest ___ szkoła.", ["nowy", "nowa", "nowe"], "nowa"),
    choice("Wybierz poprawnie: To jest ___ biuro.", ["duży", "duża", "duże"], "duże"),
    input("Uzupełnij zaimek: To jest ___ książka (ten).", "ta"),
    input("Uzupełnij zaimek: To jest ___ mieszkanie (ten).", "to"),
...
function genGenitiveVariety() {
  return cap50([
    ...
    input("Uzupełnij formę: dziś nie ma (wolny termin).", "wolnego terminu"),
...
function genDativeVariety() {
  return cap50([
    input("Uzupełnij formę: tłumaczę zadanie (nowy student).", "nowemu studentowi"),
    input("Uzupełnij formę: wysyłam wiadomość (moja siostra).", "mojej siostrze"),
    input("Uzupełnij formę: pokazuję drogę (starsza pani).", "starszej pani"),
...
function genLocativeVariety() {
  return cap50([
    input("Uzupełnij formę: mieszkam w (spokojna dzielnica).", "spokojnej dzielnicy"),
    input("Uzupełnij formę: rozmawiamy o (ważny egzamin).", "ważnym egzaminie"),
    input("Uzupełnij formę: czytam o (nowa praca).", "nowej pracy"),
...
function genPresentVariety() {
  return cap50([
    input("Uzupełnij formę: rano ja (pić) kawę przed pracą.", "piję"),
    input("Uzupełnij formę: wieczorem my (oglądać) serial po kolacji.", "oglądamy"),
    input("Uzupełnij formę: w pracy oni (mówić) po polsku i po angielsku.", "mówią"),
    input("Uzupełnij formę: w domu ty (robić) zakupy online.", "robisz"),
    input("Uzupełnij formę: na kursie wy (uczyć się) nowych słów.", "uczycie się"),
    input("Uzupełnij formę: codziennie on (czytać) wiadomości rano.", "czyta"),
    input("Przetłumacz na polski: Я пью кофе перед работой.", "piję kawę przed pracą"),
    input("Przetłumacz na polski: Они говорят по-польски и по-английски.", "mówią po polsku i po angielsku"),
    input("Przetłumacz na polski: Ты делаешь покупки онлайн.", "robisz zakupy online"),
    input("Popraw zdanie: Oni pracuje w biurze.", "oni pracują w biurze"),
    input("Popraw zdanie: Ty uczę się codziennie.", "ty uczysz się codziennie"),
    input("Popraw zdanie: My mieszka blisko centrum.", "my mieszkamy blisko centrum"),
    choice("Dla osoby `oni` wybierz poprawną formę czasownika `mówić`.", ["mówią", "mówi", "mówicie"], "mówią"),
    choice("Dla osoby `ty` wybierz poprawną formę czasownika `uczyć się`.", ["uczysz się", "uczę się", "uczymy się"], "uczysz się"),
...
function genAccusativePlural() {
  return takeExerciseSlot(getPluralCasePool("accusativePlural"), 0, EXERCISE_MAX);
}
function genGenitivePlural() {
  return takeExerciseSlot(getPluralCasePool("genitivePlural"), 0, EXERCISE_MAX);
}
function genDativePlural() {
  return takeExerciseSlot(getPluralCasePool("dativePlural"), 0, EXERCISE_MAX);
}
function genInstrumentalPlural() {
  return takeExerciseSlot(getPluralCasePool("instrumentalPlural"), 0, EXERCISE_MAX);
}
function genLocativePlural() {
  return takeExerciseSlot(getPluralCasePool("locativePlural"), 0, EXERCISE_MAX);
}
function genPresent() {
...
function genGrammarNuance(topic) {
  const blocks = {
    ...
    accusativePlural: [
      note("Почему `widzę dobrych studentów`, ale `widzę dobre książki`", "В biernik liczby mnogiej сначала решаем только одну вещь: `męskoosobowy` или `niemęskoosobowy`.\n\n1. Если это мужчины / смешанная группа людей, форма часто идёт по линии `dobrych studentów`, `nowych pracowników`.\n2. Если это вещи, документы, города, дети, книги, фильмы, очень часто biernik равен mianownik: `dobre książki`, `nowe dokumenty`, `małe dzieci`.\n\nТо есть мы не угадываем окончание вслепую. Мы сначала выбираем группу." ),
      choice("Что правильно для męskoosobowy?", ["widzę dobrych studentów", "widzę dobre studenty", "widzę dobrymi studentami"], "widzę dobrych studentów"),
    ...
    genitivePlural: [
      note("Почему `potrzebuję nowych dokumentów`", "Dopełniacz liczby mnogiej очень часто приходит после `potrzebuję`, `szukam`, `używam`, `nie ma`, `dużo`, `mało`.\n\nМы сразу видим кусок целиком: `nowe dokumenty -> nowych dokumentów`, `dobre książki -> dobrych książek`, `wolne miejsca -> wolnych miejsc`." ),
      choice("Что правильно после `nie ma`?", ["nie ma wolnych miejsc", "nie ma wolne miejsca", "nie ma wolnymi miejscami"], "nie ma wolnych miejsc"),
      choice("Что правильно после `szukam`?", ["szukam tanich biletów", "szukam tanie bilety", "szukam tanim biletom"], "szukam tanich biletów")
    ],
    dativePlural: [
      note("Почему `pomagam nowym studentom`", "Celownik liczby mnogiej — это адресат во множественном числе. Если действие направлено не на объект, а на группу получателей, очень часто появляется `-om`: `studentom`, `dzieciom`, `klientom`, `rodzicom`." ),
      choice("Что правильно после `pomagam`?", ["pomagam chorym ludziom", "pomagam chorych ludzi", "pomagam chorymi ludźmi"], "pomagam chorym ludziom"),
      choice("Что правильно после `wysyłam wiadomość`?", ["wysyłam wiadomość klientom", "wysyłam wiadomość klientów", "wysyłam wiadomość klientami"], "wysyłam wiadomość klientom")
    ],
    instrumentalPlural: [
      note("Почему `pracuję z ludźmi`", "Narzędnik liczby mnogiej часто живёт в блоках `z kim?`, `interesuję się czym?`, `zajmuję się czym?`.\n\nПолезные куски: `z ludźmi`, `z klientami`, `językami`, `projektami`, `dziećmi`." ),
      choice("Что правильно после `rozmawiam z`?", ["rozmawiam z klientami", "rozmawiam z klientów", "rozmawiam z klienci"], "rozmawiam z klientami"),
      choice("Что правильно после `interesuję się`?", ["interesuję się językami", "interesuję się języków", "interesuję się języki"], "interesuję się językami")
    ],
    locativePlural: [
      note("Почему `myślę o ważnych planach`", "Miejscownik liczby mnogiej чаще всего даёт форму на `-ach`: `planach`, `sklepach`, `zajęciach`, `miastach`, `książkach`.\n\nПредлоги `o`, `w`, `na`, `po`, `przy` очень часто ведут именно сюда." ),
      choice("Что правильно после `o`?", ["o ważnych sprawach", "o ważne sprawy", "o ważnymi sprawami"], "o ważnych sprawach"),
      choice("Что правильно после `na` в значении `gdzie?`?", ["na ciekawych zajęciach", "na ciekawe zajęcia", "na ciekawymi zajęciami"], "na ciekawych zajęciach")
    ],
    verbsPresent: [
  ...
  return uniqueExerciseItems([...(blocks[topic] || []), ...genGrammarNuancePractice(topic)]).slice(0, EXERCISE_MAX);
}

function genGrammarNuancePractice(topic) {
  const pick = (baseStart, ...groups) => takeExerciseSlot(uniqueExerciseItems(groups.flat()), baseStart, EXERCISE_MAX - 2);
  const map = {
    pluralNominative: pick(0, genPluralVariety(), genPluralAdjectives(), genOniOne(), genPluralMistakes()),
    accusativePlural: pick(0, getPluralCaseNuancePool("accusativePlural"), genAccusativeVariety(), genAccusativeAdjectives()),
    accusative: pick(12, genAccusativeVariety(), genAccusativeForms(), genAccusativeAdjectives()),
    genitivePlural: pick(0, getPluralCaseNuancePool("genitivePlural"), genGenitiveVariety(), genGenitive()),
    genitive: pick(12, genGenitiveVariety(), genGenitive()),
    dativePlural: pick(0, getPluralCaseNuancePool("dativePlural"), genDativeVariety(), genDative()),
    dative: pick(12, genDativeVariety(), genDative()),
    instrumentalPlural: pick(0, getPluralCaseNuancePool("instrumentalPlural"), genInstrumental(), genInstrumental()),
    instrumental: pick(12, genInstrumental(), genInstrumental()),
    locativePlural: pick(0, getPluralCaseNuancePool("locativePlural"), genLocativeVariety(), genLocative()),
    locative: pick(12, genLocativeVariety(), genLocative()),
    verbsPresent: pick(0, genPresentConjugationDrills(), genPresentVariety(), genPresent()),
    verbsPast: pick(8, genPast(), genPast()),
    verbsFuture: pick(0, genFutureVariety(), genFuture(), genFutureWithTime()),
    aspect: pick(0, genAspectVariety(), genAspectChoice(), genAspectPairs()),
    prepositions: pick(0, genPrepositionVariety(), genPrepositions()),
    pronouns: pick(0, genPronouns()),
    reflexiveSie: pick(0, genReflexiveSie()),
    comparisons: pick(0, genComparisons(), genComparisonVariety()),
    wordOrder: pick(0, genWordOrder(), genWordOrderVariety()),
    complexSentences: pick(0, genComplexSentences(), genComplexSentenceVariety())
  };
  return map[topic] || [];
}
...
function genGrammarStepByStep(topic) {
  const blocks = {
    ...
    accusativePlural: [
      note("Разбор по шагам: `widzę dobrych studentów`", "1. Глагол `widzę` требует `biernik`.\n2. Базовая форма: `dobrzy studenci`.\n3. Это `męskoosobowy`, значит biernik liczby mnogiej идёт по линии `dobrych studentów`.\n4. Прилагательное: `dobrzy -> dobrych`.\n5. Существительное: `studenci -> studentów`.\n\nИтог: `widzę dobrych studentów`.\n\nСравни с `widzę dobre książki`, где группа уже `niemęskoosobowy` и форма часто совпадает с mianownik." ),
      input("Uzupełnij: mam (nowe dokumenty)", "nowe dokumenty", "biernik liczby mnogiej niemęskoosobowej"),
      choice("Почему `nowe dokumenty`, а не `nowych dokumentów` после `mam`?", ["bo to biernik liczby mnogiej niemęskoosobowej", "bo po `mam` всегда jest dopełniacz", "bo dokumenty są w celowniku"], "bo to biernik liczby mnogiej niemęskoosobowej")
    ],
    accusative: [
    ...
    genitivePlural: [
      note("Разбор по шагам: `potrzebuję nowych dokumentów`", "1. `potrzebuję` требует `dopełniacz`.\n2. Базовая форма: `nowe dokumenty`.\n3. Прилагательное: `nowe -> nowych`.\n4. Существительное: `dokumenty -> dokumentów`.\n\nИтог: `potrzebuję nowych dokumentów`." ),
      input("Uzupełnij: nie ma (wolne miejsca)", "wolnych miejsc", "dopełniacz liczby mnogiej po negacji"),
      choice("Почему `dużo ludzi`, а не `dużo ludzie`?", ["bo słowo ilości wymaga dopełniacza liczby mnogiej", "bo `ludzie` są w bierniku", "bo это forma czasu przyszłego"], "bo słowo ilości wymaga dopełniacza liczby mnogiej")
    ],
    dativePlural: [
      note("Разбор по шагам: `wysyłam wiadomość nowym klientom`", "1. `wysyłam wiadomość` задаёт вопрос `komu?`.\n2. Получатель — это группа клиентов.\n3. Значит, нужен `celownik liczby mnogiej`.\n4. Прилагательное: `nowi -> nowym`.\n5. Существительное: `klienci -> klientom`.\n\nИтог: `wysyłam wiadomość nowym klientom`." ),
      input("Uzupełnij: pomagam (moi rodzice)", "moim rodzicom", "celownik liczby mnogiej"),
      choice("Почему `dzieciom`, а не `dzieci` после `daję`?", ["bo `daję` wymaga odpowiedzi na `komu?`", "bo `dzieci` всегда jest biernik", "bo это narzędnik"], "bo `daję` wymaga odpowiedzi na `komu?`")
    ],
    instrumentalPlural: [
      note("Разбор по шагам: `rozmawiam z miłymi sąsiadami`", "1. Предлог `z` в значении `с кем?` требует `narzędnik`.\n2. Базовая форма: `mili sąsiedzi`.\n3. Прилагательное: `mili -> miłymi`.\n4. Существительное: `sąsiedzi -> sąsiadami`.\n\nИтог: `rozmawiam z miłymi sąsiadami`." ),
      input("Uzupełnij: interesuję się (obce języki)", "obcymi językami", "narzędnik liczby mnogiej"),
      choice("Почему `z ludźmi`, а не `z ludzi`?", ["bo `z kim?` daje narzędnik", "bo `ludzi` это mianownik", "bo `z` zawsze daje dopełniacz"], "bo `z kim?` daje narzędnik")
    ],
    locativePlural: [
      note("Разбор по шагам: `myślę o ważnych planach`", "1. Предлог `o` требует `miejscownik`.\n2. Базовая форма: `ważne plany`.\n3. Прилагательное: `ważne -> ważnych`.\n4. Существительное: `plany -> planach`.\n\nИтог: `myślę o ważnych planach`." ),
      input("Uzupełnij: jestem na (ciekawe zajęcia)", "ciekawych zajęciach", "miejscownik liczby mnogiej"),
      choice("Почему `w sklepach`, а не `w sklepy`?", ["bo `w` w znaczeniu `gdzie?` wymaga miejscownika", "bo `sklepy` это biernik", "bo это narzędnik liczby mnogiej"], "bo `w` w znaczeniu `gdzie?` wymaga miejscownika")
    ],
    verbsPresent: [
  ...
  return uniqueExerciseItems([...(blocks[topic] || []), ...genGrammarStepByStepPractice(topic)]).slice(0, EXERCISE_MAX);
}

function genGrammarStepByStepPractice(topic) {
  const pick = (baseStart, ...groups) => takeExerciseSlot(uniqueExerciseItems(groups.flat()), baseStart, EXERCISE_MAX - 2);
  const map = {
    pluralNominative: pick(20, genPluralVariety(), genMascPlural(), genNonMascPlural(), genPluralAdjectives(), genOniOne()),
    accusativePlural: pick(0, getPluralCaseStepPool("accusativePlural"), genAccusativeForms(), genAccusativeVariety()),
    accusative: pick(24, genAccusativeAdjectives(), genAccusativeForms(), genAccusativeVariety()),
    genitivePlural: pick(0, getPluralCaseStepPool("genitivePlural"), genGenitive(), genGenitiveVariety()),
    genitive: pick(24, genGenitive(), genGenitiveVariety()),
    dativePlural: pick(0, getPluralCaseStepPool("dativePlural"), genDative(), genDativeVariety()),
    dative: pick(24, genDative(), genDativeVariety()),
    instrumentalPlural: pick(0, getPluralCaseStepPool("instrumentalPlural"), genInstrumental(), genInstrumental()),
    instrumental: pick(24, genInstrumental(), genInstrumental()),
    locativePlural: pick(0, getPluralCaseStepPool("locativePlural"), genLocative(), genLocativeVariety()),
    locative: pick(24, genLocative(), genLocativeVariety()),
    verbsPresent: pick(24, genPresentVariety(), genPresentConjugationDrills(), genPresent()),
    irregularVerbs: pick(24, genIrregularVerbsVariety(), genPresentConjugationDrills(), genPresent()),
    verbsPast: pick(16, genPast()),
    verbsFuture: pick(24, genFutureVariety(), genFuture(), genFutureWithTime()),
    aspect: pick(24, genAspectVariety(), genAspectChoice(), genAspectPairs()),
    prepositions: pick(20, genPrepositionVariety(), genPrepositions()),
    numbersTime: pick(20, genNumbers(), genMoney(), genNumberRules(), genClock(), genTimePhrases(), genNumberTimeVariety()),
    complexSentences: pick(20, genComplexSentences(), genComplexSentenceVariety()),
    politeConditional: pick(8, genPoliteConditional()),
    imperatives: pick(8, genImperatives()),
    pronouns: pick(8, genPronouns()),
    reflexiveSie: pick(8, genReflexiveSie()),
    comparisons: pick(20, genComparisons(), genComparisonVariety()),
    modalVerbs: pick(20, genModalVerbs(), genModalVariety()),
    impersonal: pick(20, genImpersonal(), genImpersonalVariety()),
    wordOrder: pick(20, genWordOrder(), genWordOrderVariety()),
    b1Connectors: pick(20, genB1Connectors(), genConnectorChoiceAdvanced()),
    b1Mistakes: pick(20, genB1Mistakes(), genExamGrammarSkills(), genSentenceAssemblyB1())
  };
  return map[topic] || [];
}
...
const topicGoals = {
  ...
  genitivePlural: ["Уверенно строить формы после `nie ma`, `dużo`, `szukam`, `potrzebuję`", "Узнавать модели `-ów`, `-i/-y`, zero ending", "Учить dopełniacz plural как отдельный навык"],
  genitive: ["Строить отрицание nie mam / nie ma", "Использовать dużo, mało, trochę", "Говорить о том, чего не хватает"],
  dativePlural: ["Говорить кому даёшь, помогаешь и объясняешь во множественном числе", "Автоматически узнавать модель `-om`", "Не путать адресата с прямым объектом"],
  dative: ["Говорить кому помогаешь и что даёшь", "Использовать mi, ci, mu, jej", "Не подставлять biernik после pomagać"],
  instrumentalPlural: ["Уверенно говорить `z kim? z czym?` во множественном числе", "Использовать формы `-ami / -mi`", "Закрепить конструкции `z`, `interesować się`, `zajmować się`"],
  instrumental: ["Говорить с кем и чем", "Называть профессию после jestem", "Использовать z + narzędnik"],
  locativePlural: ["Уверенно строить формы на `-ach`", "Говорить о темах, местах и занятиях во множественном числе", "Связывать `o / w / na / po / przy` с miejscownik plural"],
...
function getTopicSupplementPool(topicKey) {
  ...
  const pools = {
    ...
    accusative: [...genGrammarNuance("accusative"), ...genGrammarStepByStep("accusative"), ...genWorkbookDrills("accusative"), ...genAccusativeForms(), ...genAccusativeAdjectives(), ...genAccusativeVariety()],
    ...
    genitive: [...genGrammarNuance("genitive"), ...genGrammarStepByStep("genitive"), ...genWorkbookDrills("genitive"), ...genGenitive(), ...genGenitiveVariety()],
    ...
    dative: [...genGrammarNuance("dative"), ...genGrammarStepByStep("dative"), ...genWorkbookDrills("dative"), ...genDative(), ...genDativeVariety()],
    ...
    instrumental: [...genGrammarNuance("instrumental"), ...genGrammarStepByStep("instrumental"), ...genWorkbookDrills("instrumental"), ...genInstrumental()],
    ...
    locative: [...genGrammarNuance("locative"), ...genGrammarStepByStep("locative"), ...genWorkbookDrills("locative"), ...genLocative(), ...genLocativeVariety()],
...
function dedupeTopicExercises(topicKey, topic) {
  ...
  const exercises = (topic.exercises || [])
    .filter(Boolean)
    .map((exercise) => {
      const items = (exercise.items || []).filter((item) => {
        if (!isPracticeItem(item)) return true;
...
function materializeTopic(topic) {
  if (!topic) return topic;
  if (topic.exercises) return { ...topic, exercises: (topic.exercises || []).filter(Boolean) };
  if (typeof topic.buildExercises === "function") {
...
function AnswerBlock({ item, state, setState }) {
  if (item.type === "note") {
    return (
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const nextTopicKey = rawTopics[course.topicKey] ? course.topicKey : topicKeys[0];
    const nextTopic = getTopic(nextTopicKey);
    const nextExerciseIndex = nextTopic.exercises[course.exerciseIndex] ? course.exerciseIndex : 0;
  ...
  }, [activeProfileId, loadedProfileId, topicKeys, currentVariant]);

  useEffect(() => {
    if (activeProfileId !== loadedProfileId) return;
    saveProfileCourse(activeProfileId, { topicKey: safeTopicKey, exerciseIndex: safeExerciseIndex, answers, review: {} });
  }, [answers, safeTopicKey, safeExerciseIndex, activeProfileId, loadedProfileId]);
  ...
  function prev() {
    if (safeExerciseIndex > 0) {
      selectExercise(safeTopicKey, safeExerciseIndex - 1);
      return;
    }
    const prevTopicKey = topicKeys[currentTopicPosition - 1];
    if (!prevTopicKey) return;
    const prevTopic = getTopic(prevTopicKey);
    selectExercise(prevTopicKey, Math.max(0, prevTopic.exercises.length - 1));
  }
  function next() {
    if (safeExerciseIndex < topic.exercises.length - 1) {
      selectExercise(safeTopicKey, safeExerciseIndex + 1);
      return;
    }
    const nextTopicKey = topicKeys[currentTopicPosition + 1];
    if (!nextTopicKey) return;
    selectExercise(nextTopicKey, 0);
  }
              ...
              {!topicLoading && currentItems.map((item, i) => {
                ...
                return (
                  <div key={id} style={styles.item}>
                    ...
                    <AnswerBlock item={item} state={answers[id]} setState={(v) => setAnswer(id, v)} />
                  </div>
                );
              })}

              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 18 }}>
                <button onClick={prev} disabled={currentTopicPosition === 0 && safeExerciseIndex === 0} style={{ ...styles.btn, opacity: currentTopicPosition === 0 && safeExerciseIndex === 0 ? 0.5 : 1 }}>← Предыдущее</button>
                <button onClick={next} disabled={currentTopicPosition === topicKeys.length - 1 && safeExerciseIndex === topic.exercises.length - 1} style={{ ...styles.primary, opacity: currentTopicPosition === topicKeys.length - 1 && safeExerciseIndex === topic.exercises.length - 1 ? 0.5 : 1 }}>Следующее →</button>
              </div>
            </section>
          </main>
        </div>
